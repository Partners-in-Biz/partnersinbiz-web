...
| `firstPilotSet` | Add children visual gated pilot; remove negative-control fixture; make client-owned workbook the first proof. | Book-family gate profile, rights/provenance, package QA, ownership/account authority, portal artifact, analytics confidence, and Research evidence requirements for the changed pilot. | Approval packet, pilot product decision register, book-family gate catalog, acceptance fixtures, mock review packet, coverage audit. | V1 may prove only easy pass cases or may take on fixed-layout/rights complexity too early. | Removing blocker proof speeds the first build but weakens evidence that Book Studio can stop attractive unsafe projects. |
...
| `hermesFirstScope` | Wave 1 only; Wave 1 plus selected Wave 2 docs/fixtures; broader docs only with no runtime dispatch. | Skill manifests, input contracts, output artifacts, fixture IDs, pass/warn/block rubrics, forbidden actions, sanitizer expectations, reviewer defaults. | Approval packet, Hermes skill blueprint, Hermes evaluation packet, acceptance fixtures, review scorecard, coverage audit. | Hermes can appear authorized to publish, approve, spend, message clients, request secrets, or mark artifacts client-ready. | Hermes can create polished output quickly; without fixtures and blockers, polish hides weak evidence. |
| `ownershipModel` | PiB-owned only; shared PiB/client-owned; client-owned first proof. | Account authority, publisher/imprint, payment/revenue separation, client approval artifacts, cost ledger, rights assignment, tax/reporting boundary. | Approval packet, ownership/commercial model, jurisdiction/local publisher model, publishing/analytics model, mock review packet. | Revenue, rights, account access, and client approval can blur into one unsafe operating lane. | Shared ownership expands opportunity, but it adds governance before the module has proven one clean owner path. |
...
| `productionReadinessScope` | Package QA evidence only; add checksum-bound readiness; add manual preview rendering fixture; defer automated export/file validation. | File/proof/cover/rights/accessibility/source freshness evidence, checksum binding, invalidation rules, proof package versioning, manual preview fixture. | Approval packet, production package QA model, mock review packet, acceptance fixtures, review script. | A draft PDF, EPUB, or cover can be mistaken for upload-ready package evidence. | Package QA sounds like implementation detail, but it is the boundary between creative output and a safe manual handoff. |
...
| Field adds channel, pilot, portal, Hermes, publishing, or analytics scope. | V1 becomes broader and source-sensitive. | Refresh affected sources and update affected appendices before planning. |
