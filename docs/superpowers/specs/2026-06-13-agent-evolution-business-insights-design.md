...
- Added an ads business-signal adapter in `lib/loop-engine/ads-business-signals.ts` for unhealthy ad connections and paid media campaigns waiting for review. The live collector now merges these ads signals into `business-insight-review` drafts.
- Added ads metric refresh support for due business-insight action measurement. `ads_connections_unhealthy` and `ads_campaigns_waiting_review` can now populate `metadata.businessInsightAction.latest` directly from ads source data before outcome comparison.
- Added an SEO business-signal adapter in `lib/loop-engine/seo-business-signals.ts` for blocked SEO tasks and high-severity sprint health signals. The live collector now merges these SEO signals into `business-insight-review` drafts.
- Added SEO metric refresh support for due business-insight action measurement. `seo_blocked_tasks` and `seo_high_severity_signals` can now populate `metadata.businessInsightAction.latest` directly from SEO source data before outcome comparison.
- Reviewed 0xwhrrari's 2026-06-12 "Loop Engineering" article. The extra implementation insight was loop economics: loops that approach or exceed budget are not just runtime facts, they are agent-evolution signals.
- Added loop budget drift collection to `lib/loop-engine/live-signal-collector.ts`. Recent `loop_engine_runs` with `observability.budgetStatus` of `near-limit` or `exceeded` now produce `tooling-gap` agent-evolution signals pointing back to the loop run, so stop conditions, model routing, context packing, or verifier fanout can be reviewed before costs drift.
- Added invoice business-signal coverage in `lib/loop-engine/invoice-business-signals.ts` for overdue value, payment proofs needing review, draft invoice value waiting to send, and partially-paid outstanding value. Invoice signals stay finance-gated and internal-review-only.
- Added invoice metric refresh support and source-specific refresh for `high_risk_blocked_task` project-risk action outcomes, so due outcome measurement can prove whether the original invoice/project-risk metric improved.
- Preserved supplied `BusinessInsightSignal.approvalGate` values through `POST /api/v1/admin/loop-engine/evaluate`, so explicit finance-gated signals do not degrade to generic human review.
- Added persisted duration/count telemetry for saved admin evaluate runs and persisted cron collection runs. Run records now include `usage.durationMs`, `runtime`, and `telemetry` blocks so loop economics are visible to the review collector even when exact token/cost data is unavailable.
- Added exact model token/cost telemetry capture for Hermes watcher task dispatches. `services/agent-watcher/src/hermes.ts` now normalizes upstream `usage`, token, model, reasoning-effort, and billing/cost fields from terminal Hermes run payloads; `services/agent-watcher/src/watcher.ts` persists the normalized telemetry on task output and writes an `agent-task-dispatch` record to `loop_engine_runs`.
- When Hermes does not expose exact token or cost values, the watcher now persists a structured unavailable state (`tokenSource: unavailable`, `costSource: unavailable`, `missing: ['token_usage', 'cost_usd']`) instead of estimating or inventing numbers. `lib/loop-engine/live-signal-collector.ts` turns those records into `tooling-gap` agent-evolution signals so upstream runtime support can be improved deliberately.
- Still pending: source adapters for any future commercial modules not yet covered by CRM/support/social/ads/SEO/invoices/project-risk tasks and a trend dashboard for loop economics.
