...
| Metadata and store listing | Title/subtitle, description, KDP keywords/categories, Google genres, contributors, series, identifiers, language, translated-edition metadata, and portal listing summaries are channel-specific, source-backed, rights-aware, and version-bound. | One channel or metadata lane has owner/date warning, narrower portal wording, or source-refresh task while dependent claims stay limited. | Packet readiness, portal listing summary, launch copy, or analytics promotion can proceed with keyword stuffing, misleading category/genre, copied competitor copy, unsupported contributor/series/identifier state, KDP/Google flattening, or rank/sales/bestseller promises. | Metadata/listing model, source refresh contract, market evidence model, publishing and analytics model, acceptance fixtures. |
...
- The metadata/listing gate prevents channel packets and portal summaries from using misleading titles, categories, keywords, copied competitor copy, unsupported contributor or series claims, KDP/Google flattening, or rank promises.
...
Use the revision impact matrix before planning from any warning that changes `productPosture`, `firstChannels`, `firstPilotSet`, `firstPortalReviewArtifacts`, `marketEvidenceScope`, `metadataListingScope`, `hermesFirstScope`, `ownershipModel`, `jurisdictionAndLocalPublisherScope`, `productionReadinessScope`, `analyticsScope`, or `acceptedDeferrals`.
