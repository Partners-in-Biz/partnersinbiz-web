...
| Sender identities and deterministic policies | Complete | sender store/resolution/context, scoped APIs, broadcast and sequence integration; table tests green | Signed-in UI QA and provider-backed identity/domain proof remain. |
...
| Marketing Studio IA | Partial | `/portal/marketing`, dashboard/nav/program list and tests exist | Planned V2 per-org rollout flag is absent; all linked surfaces need signed-in context-propagation QA. |
