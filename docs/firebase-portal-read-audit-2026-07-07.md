...
| `GET /api/v1/social/accounts` | `social_accounts` | Not yet. | In-memory count. | Needs a schema decision for organization accounts because org-level rows currently use missing `accountScope`, while personal rows use `accountScope: personal`. Dashboard no longer calls it on the normal path. |
| `GET /api/v1/client-documents` | `client_documents` | Not yet. | In-memory count. | Platform-linked document visibility still scans platform docs; optimize with linked org indexes in a separate pass. |
| `GET /api/v1/projects/reporting` | `projects` subcollections | Not yet. | N/A. | Explicit deep report; should stay off page initial load or be lazy/user-triggered. |
