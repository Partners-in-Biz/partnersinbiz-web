...
requireText('macOS heartbeat watchdog', plist, /__PIB_RUNTIME_BINARY__<\/string><string>supervise/)
...
requireText('Linux systemd service', linuxService, /User=root[\s\S]*ExecStart=\/opt\/partnersinbiz\/current\/pib-runtime supervise/)
...
requireText('runtime external heartbeat watchdog', runtimeCli, /superviseRuntimeService/)
...
requireText('Windows heartbeat watchdog', read('runtime-installers/windows/PartnersInBizRuntimeService.cs'), /RuntimeInfo\("supervise"\)/)
...
requireText('Windows supervisor tree stop', read('runtime-installers/windows/PartnersInBizRuntimeService.cs'), /Kill\(true\)/)
...
requireText('Windows internal machine root trust', windowsBootstrap, /@\('Root','TrustedPublisher'\)/)
...
requireText('Windows internal private-root-only failure', internalWindowsWorkflow, /UnknownError[\s\S]*0x800B0109/)
requireText('Windows internal Authenticode tamper control', internalWindowsWorkflow, /TamperedSignature\.Status -ne 'HashMismatch'/)
