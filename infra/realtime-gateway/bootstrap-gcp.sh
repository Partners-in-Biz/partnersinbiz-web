...
if [[ "$APPLY" != true ]]; then
  echo "Preflight only. Re-run with --apply plus PIB_REALTIME_NETWORK and PIB_REALTIME_SUBNET_RANGE after confirming the VPC/CIDR."
  exit 0
fi
...
gcloud services enable run.googleapis.com pubsub.googleapis.com redis.googleapis.com artifactregistry.googleapis.com cloudbuild.googleapis.com cloudfunctions.googleapis.com eventarc.googleapis.com --project="$PROJECT_ID"
...
gcloud redis instances describe "$REDIS_INSTANCE" --region="$REGION" --project="$PROJECT_ID" >/dev/null 2>&1 || \
  gcloud redis instances create "$REDIS_INSTANCE" --tier=standard --size=1 --region="$REGION" --network="$NETWORK" --redis-version=redis_7_2 --project="$PROJECT_ID"

gcloud projects add-iam-policy-binding "$PROJECT_ID" --member="serviceAccount:$PUBLISHER_SA" --role="roles/pubsub.publisher"
gcloud run services add-iam-policy-binding "$GATEWAY_SERVICE" --member="serviceAccount:$PUSH_SA" --role="roles/run.invoker" --region="$REGION" --project="$PROJECT_ID" 2>/dev/null || true
...
gcloud run deploy "$GATEWAY_SERVICE" --image="$IMAGE" --region="$REGION" --project="$PROJECT_ID" --allow-unauthenticated \
  ...
  --set-env-vars="REDIS_URL=redis://$(gcloud redis instances describe "$REDIS_INSTANCE" --region="$REGION" --project="$PROJECT_ID" --format='value(host)'):6379,PUBSUB_PUSH_SERVICE_ACCOUNT=$PUSH_SA,PUBSUB_AUDIENCE=https://placeholder.invalid,ALLOWED_ORIGINS=https://partnersinbiz.online"
...
gcloud pubsub subscriptions describe "$SUBSCRIPTION" --project="$PROJECT_ID" >/dev/null 2>&1 || \
  gcloud pubsub subscriptions create "$SUBSCRIPTION" --topic="$TOPIC" --push-endpoint="$GATEWAY_URL/internal/events/pubsub" \
    --push-auth-service-account="$PUSH_SA" --dead-letter-topic="$DLQ_TOPIC" --max-delivery-attempts=10 --project="$PROJECT_ID"
