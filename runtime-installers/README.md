...
Unsigned packages are never silently accepted. macOS requires `PIB_ALLOW_UNSIGNED_DEV=1`; Windows requires `-AllowUnsignedDev`, and both print a prominent warning. This mode is development-only. The Ed25519 update-signing key is configured, but customer macOS publication remains blocked on Developer ID Application and Developer ID Installer certificates plus notarisation credentials. Windows publication remains blocked on an Authenticode certificate/private-key signing service. Those OS trust gates are separate from the release-manifest signature and may not be bypassed.
