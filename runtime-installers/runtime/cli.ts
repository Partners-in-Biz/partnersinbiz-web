...
const runtimeVersion=process.env.PIB_RUNTIME_VERSION||'1.1.7'
...
async function service(){let stopped=false;process.once('SIGTERM',()=>{stopped=true});process.once('SIGINT',()=>{stopped=true});if(fs.existsSync(revocationMarker)){await recoverPendingRevocation(signedRemoteRevoke,clearRevocation,ms=>new Promise(r=>setTimeout(r,ms)),()=>stopped);return}const pollers=[()=>pollForever(claim,run,()=>stopped),()=>pollWorkbenchForever(workbenchClaim,workbenchRun,()=>stopped),()=>pollAgentHostForever(agentHostClaim,agentHostRun,()=>stopped),()=>heartbeatForever(heartbeat,()=>stopped)];if(nativeWorkspaceSyncSupported())pollers.push(()=>pollWorkspaceSyncForever(syncClaim,syncRun,syncFlush,async()=>undefined,()=>stopped));await runRuntimeServicePollers(...pollers)}
