...
const runtimeVersion=process.env.PIB_RUNTIME_VERSION||'1.1.23'
...
async function heartbeat(){let i=await identity();if(i.pendingRotationDeliveryId)i=await handleRotation(i,{rotation:null},persistIdentity,identity,acknowledgeRotation);const hermesProbe=await probeLocalHermes();linkedRunCapacity.setHealthyAgentIds(hermesProbe.availableAgentIds);const data=await new DeviceApiClient(api,i).postParsed(`/api/v1/linked-computers/${i.deviceId}/heartbeat`,linkedRuntimeHeartbeatBody(process.platform,hermesProbe),jsonData);await handleRotation(i,data,persistIdentity,identity,acknowledgeRotation)}
...
async function waitForAgentIdle(agentId:string,timeoutMs:number){const deadline=Date.now()+timeoutMs;while(Date.now()<deadline){if(linkedRunCapacity.activeCount(agentId)===0)return true;await new Promise(r=>setTimeout(r,500))}return false}
