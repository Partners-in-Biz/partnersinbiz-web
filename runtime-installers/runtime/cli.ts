...
async function pair(challengeId:string,releaseChannel:'internal'|'stable'='stable'){
...
export async function main(argv=process.argv.slice(2)){const [cmd,...args]=argv;if(cmd==='pair')return pair(option(args,'--challenge'),releaseChannelFrom(args));if(cmd==='service'||cmd==='bridge')return service();if(cmd==='supervise')return supervise();if(cmd==='heartbeat')return heartbeat();if(cmd==='map'){const id=option(args,'--mapping');maps.map(id,option(args,'--folder'));await confirmMapping(id,true);return}if(cmd==='unmap'){const id=option(args,'--mapping');maps.unmap(id);await confirmMapping(id,false);return}if(cmd==='status'){process.stdout.write(JSON.stringify({paired:await store.get('identity').then(()=>true,()=>false),mappings:maps.status()})+'\n');return}if(cmd==='revoke')return revoke();throw new Error('usage: pib-runtime pair|supervise|service|heartbeat|map|unmap|status|revoke')}
