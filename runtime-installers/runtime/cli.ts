...
export async function isRevokeAcknowledged(response:Response){if(!response.ok)return false;try{const body=await response.json() as any;return body?.revoked===true&&['device_revoked','already_revoked'].includes(body.code)}catch{return false}}
