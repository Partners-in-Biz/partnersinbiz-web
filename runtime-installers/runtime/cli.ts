...
async function pair(challengeId:string){
 ...
 const data=await jsonData(response),outboundIdentity=sanitizeIdentity({...data,privateKey:keys.privateKey.export({type:'pkcs8',format:'pem'})});await persistIdentity(outboundIdentity);await heartbeat(true);process.stdout.write('Paired.\n')
}
...
async function heartbeat(bootstrapTransport=false){let i=await identity();if(i.pendingRotationDeliveryId)i=await handleRotation(i,{rotation:null},persistIdentity as any,identity as any,acknowledgeRotation as any);const response=await new DeviceApiClient(api,i).post(`/api/v1/linked-computers/${i.deviceId}/heartbeat`,{runtimeVersion,health:'ok',capabilities:['workspace.execute'],bootstrapTransport,claimRotation:true}),data=await jsonData(response);await handleRotation(i,data,persistIdentity as any,identity as any,acknowledgeRotation as any)}
