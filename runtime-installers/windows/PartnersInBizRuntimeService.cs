...
namespace PartnersInBiz {
  public sealed class RuntimeService : ServiceBase {
    ...
    void Supervise(CancellationToken token){var backoff=1000;while(!token.IsCancellationRequested){try{var process=Process.Start(RuntimeInfo("service"));lock(childLock)child=process;while(!token.IsCancellationRequested&&!process.WaitForExit(500)){}if(token.IsCancellationRequested)break;EventLog.WriteEntry("Runtime worker exited; restarting.",EventLogEntryType.Warning);backoff=Math.Min(backoff*2,30000);}catch(Exception e){EventLog.WriteEntry("Runtime worker start failed: "+e.GetType().Name,EventLogEntryType.Error);backoff=Math.Min(backoff*2,30000);}if(token.WaitHandle.WaitOne(backoff))break;}lock(childLock)child=null;}
    ...
    void ClaimPairing(){string ready=Path.Combine(root,"pairing.ready"),claim=Path.Combine(root,"pairing.claimed");try{if(!File.Exists(ready))return;File.Move(ready,claim);var plain=ProtectedData.Unprotect(File.ReadAllBytes(claim),null,DataProtectionScope.LocalMachine);File.Delete(claim);var handoff=JsonSerializer.Deserialize<Handoff>(plain);CryptographicOperations.ZeroMemory(plain);var pair=Process.Start(RuntimeInfo("pair --challenge "+handoff.challengeId,true));pair.StandardInput.WriteLine(handoff.code);pair.StandardInput.Close();if(!pair.WaitForExit(30000)||pair.ExitCode!=0)throw new InvalidOperationException("pairing failed");RestartWorker();}catch(Exception e){EventLog.WriteEntry("Pairing handoff failed: "+e.GetType().Name,EventLogEntryType.Error);try{File.Delete(claim);}catch{}}}
