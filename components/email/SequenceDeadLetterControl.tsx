  ...
  return (
    <section className="mt-3 rounded-lg border border-amber-400/20 bg-amber-400/[0.05] p-3" aria-label="Journey delivery failures">
      <div className="mb-2">
        ...
        <p className="text-[11px] text-on-surface-variant">Review and safely requeue the current step.</p>
      </div>
        ...
        {items.map((item) => (
          <div key={item.id} className="flex items-center justify-between gap-2 rounded border border-[var(--color-card-border)] px-2 py-1.5 text-xs">
            <div className="min-w-0">
              ...
              <p className="truncate text-[11px] text-on-surface-variant">{item.deadLetter?.reason ?? 'Delivery failed'} · {item.deadLetter?.attempts ?? 0} attempts</p>
            </div>
