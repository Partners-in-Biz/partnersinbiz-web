...
export function LinkedWorkbenchFolderPreview({ conversationId, path }: { conversationId: string; path: string }) {
  ...
  return (
    <section aria-label="Linked folder contents" className="overflow-hidden rounded-xl border border-[var(--color-card-border)] bg-white/[0.025]">
      <div className="flex items-center justify-between gap-2 border-b border-[var(--color-card-border)] px-3 py-2">
        ...
        <button type="button" aria-label="Refresh linked folder" onClick={() => { void loadFolder() }} disabled={loading} className="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-[var(--color-pib-text-muted)] hover:bg-white/[0.06] disabled:opacity-50">
          <span aria-hidden="true" className={`material-symbols-outlined text-[16px] ${loading ? 'animate-spin' : ''}`}>refresh</span>
        </button>
      </div>
      <div className="h-72">
