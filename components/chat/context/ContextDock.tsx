...
export function ContextDock({ model, open, onClose, compact = false, activeArtifactId, onArtifactActivate, onAction, actionError, pendingActionId, execution, mode = 'single', onModeChange, canvasWidth = 520, onCanvasWidthChange, secondaryContext, secondaryOptions = [], onSecondaryChange }: { model: ChatContextReadModel; open: boolean; onClose: () => void; compact?: boolean; activeArtifactId?: string; onArtifactActivate?: (artifact: ChatArtifactSummary) => void; onAction?: (action: ChatContextAction) => void; actionError?: string | null; pendingActionId?: string; execution?: RuntimeExecution; mode?: 'single' | 'dual'; onModeChange?: (mode: 'single' | 'dual') => void; canvasWidth?: number; onCanvasWidthChange?: (width: number) => void; secondaryContext?: ChatContextOption; secondaryOptions?: ChatContextOption[]; onSecondaryChange?: (context: ChatContextOption) => void }) {
  ...
  useEffect(() => {
    if (!open || mode !== 'dual' || !wideDesktop || !secondaryContext) { setSecondaryModel(null); return }
    ...
    fetch(`/api/v1/chat-context/${encodeURIComponent(secondaryContext.kind)}/${encodeURIComponent(secondaryContext.id)}`, { signal: controller.signal })
      ...
      .then((body) => setSecondaryModel(body?.data ?? null))
      .catch(() => { if (!controller.signal.aborted) setSecondaryModel(null) })
    return () => controller.abort()
  }, [mode, open, secondaryContext, wideDesktop])
  useEffect(() => {
    if (!open) return
  ...
  return <>{sheet && <div aria-hidden="true" onClick={onClose} className="fixed inset-0 z-30 bg-black/55" />}<aside ref={dialogRef} tabIndex={-1} role="dialog" aria-modal={sheet ? 'true' : 'false'} aria-label={`${model.context.label} context`} data-presentation={sheet ? 'sheet' : dual ? 'dual' : 'canvas'} style={!sheet && wideDesktop ? { width: `${canvasWidth}px` } : undefined} className={sheet ? 'fixed inset-0 z-40 flex flex-col overflow-hidden border border-[var(--color-card-border)] bg-[var(--color-surface,#151515)] shadow-2xl sm:inset-x-3 sm:bottom-3 sm:top-[8%] sm:rounded-2xl' : `absolute bottom-0 right-0 top-[98px] z-40 flex w-[42%] min-w-[420px] max-w-[640px] flex-col overflow-hidden rounded-bl-xl border border-[var(--color-card-border)] bg-[var(--color-surface,#151515)] shadow-2xl xl:w-auto`}>
    ...
    <header className="flex min-h-14 items-center gap-2 border-b border-[var(--color-card-border)] px-3 py-2 pb-[max(.5rem,env(safe-area-inset-top))]"><span aria-hidden="true" className="material-symbols-outlined grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-primary/10 text-[18px] text-primary">{model.context.icon}</span><div className="min-w-0 flex-1"><p className="text-[9px] font-label uppercase tracking-[0.18em] text-[var(--color-pib-text-muted)]">{model.context.kind.replaceAll('_', ' ')}</p><h2 className="truncate text-sm font-semibold text-[var(--color-pib-text)]">{model.context.label}</h2></div>{wideDesktop && secondaryOptions.length > 0 && <button type="button" aria-label={dual ? 'Use single context canvas' : 'Use dual context canvas'} aria-pressed={Boolean(dual)} onClick={() => onModeChange?.(dual ? 'single' : 'dual')} className="hidden h-8 items-center gap-1 rounded-lg border border-[var(--color-card-border)] px-2 text-[10px] text-[var(--color-pib-text-muted)] hover:bg-white/[0.06] xl:inline-flex"><span aria-hidden="true" className="material-symbols-outlined text-[15px]">splitscreen</span>{dual ? 'Single' : 'Dual focus'}</button>}<button ref={closeRef} type="button" aria-label="Close context dock" onClick={onClose} className="inline-flex h-11 shrink-0 items-center gap-1 rounded-lg px-2 text-xs text-[var(--color-pib-text-muted)] outline-none hover:bg-white/[0.07] hover:text-[var(--color-pib-text)] focus-visible:ring-2 focus-visible:ring-primary/60 sm:h-8"><span className="material-symbols-outlined text-[17px]" aria-hidden="true">arrow_back</span><span className="sm:hidden">Back to chat</span></button></header>
    {dual && <div className="flex items-center gap-2 border-b border-[var(--color-card-border)] bg-black/10 px-3 py-2"><span className="text-[10px] font-label uppercase tracking-[0.15em] text-[var(--color-pib-text-muted)]">Second focus</span><select aria-label="Secondary context" value={`${secondaryContext.kind}:${secondaryContext.id}`} onChange={(event) => { const next = secondaryOptions.find((option) => `${option.kind}:${option.id}` === event.target.value); if (next) onSecondaryChange?.(next) }} className="min-w-0 flex-1 rounded-lg border border-[var(--color-card-border)] bg-black/20 px-2 py-1.5 text-xs text-[var(--color-pib-text)]">{secondaryOptions.map((option) => <option key={`${option.kind}:${option.id}`} value={`${option.kind}:${option.id}`}>{option.label}</option>)}</select></div>}
    <div className={`min-h-0 flex-1 overflow-y-auto p-3 ${dual ? 'grid content-start gap-3' : 'space-y-4'}`}>
      ...
      {model.context.kind === 'document' && <DocumentContextPreview documentId={model.context.id} />}
      {model.preview?.kind === 'summary' && model.preview.text && <section className="rounded-xl border border-[var(--color-card-border)] bg-white/[0.025] p-3"><h3 className="text-[10px] font-label uppercase tracking-[0.18em] text-[var(--color-pib-text-muted)]">Preview</h3><p className="mt-2 text-xs leading-relaxed text-[var(--color-pib-text)]">{model.preview.text}</p></section>}
      ...
      {model.artifacts.length > 0 && <section><h3 className="mb-2 text-[10px] font-label uppercase tracking-[0.18em] text-[var(--color-pib-text-muted)]">Artifacts</h3><div className="space-y-2">{model.artifacts.map((artifact) => <div key={artifact.id} data-active={artifact.id === activeArtifactId || undefined}><ContextArtifactCard artifact={artifact} selected={artifact.id === activeArtifactId} onActivate={onArtifactActivate} onAction={onAction} pendingActionId={pendingActionId} /></div>)}</div></section>}
      {model.activity.length > 0 && <section><h3 className="mb-2 text-[10px] font-label uppercase tracking-[0.18em] text-[var(--color-pib-text-muted)]">Activity</h3><ul>{model.activity.map((item) => <li key={item.id} className="py-1 text-[11px] text-[var(--color-pib-text-muted)]">{item.label}</li>)}</ul></section>}
      {model.context.href && <a href={model.context.href} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-primary/30 bg-primary/10 px-3 text-xs font-semibold text-primary hover:bg-primary/15 sm:h-9">Open full workspace<span aria-hidden="true" className="material-symbols-outlined text-[15px]">open_in_new</span></a>}
      {dual && <section className="rounded-xl border border-primary/20 bg-primary/[0.035] p-3"><p className="text-[9px] font-label uppercase tracking-[0.18em] text-primary">Related context</p><h3 className="mt-1 text-sm font-semibold text-[var(--color-pib-text)]">{secondaryModel?.context.label ?? secondaryContext.label}</h3><p className="mt-2 text-xs leading-relaxed text-[var(--color-pib-text-muted)]">{secondaryModel?.pulse.headline ?? secondaryContext.summary ?? 'Live details load from the authoritative workspace.'}</p>{secondaryModel?.pulse.progress && <p className="mt-3 text-xs text-[var(--color-pib-text)]">{secondaryModel.pulse.progress.complete} of {secondaryModel.pulse.progress.total} complete</p>}{(secondaryModel?.context.href ?? secondaryContext.href) && <a href={secondaryModel?.context.href ?? secondaryContext.href} className="mt-3 inline-flex h-9 items-center gap-1 rounded-lg border border-[var(--color-card-border)] px-3 text-xs text-[var(--color-pib-text)]">Open workspace<span aria-hidden="true" className="material-symbols-outlined text-[14px]">open_in_new</span></a>}</section>}
    </div>
  </aside></>
}
