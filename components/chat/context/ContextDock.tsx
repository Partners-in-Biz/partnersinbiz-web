  ...
  return (
    ...
    >
      <button
        ...
        onClick={onClear}
        className={`inline-flex min-h-8 items-center gap-1 rounded-[4px] border px-2 py-0.5 text-[10px] font-medium outline-none focus-visible:ring-2 focus-visible:ring-primary/60 ${ allActive ? 'border-primary/40 bg-primary/15 text-primary' : 'border-white/10 bg-white/[0.03] text-[var(--color-pib-text-muted)] hover:bg-white/[0.06]' }`}
      >
        All
      ...
      {counts.map((row) => {
        ...
        return (
          <button
            ...
            className={`inline-flex min-h-8 items-center gap-1 rounded-[4px] border px-2 py-0.5 text-[10px] font-medium outline-none focus-visible:ring-2 focus-visible:ring-primary/60 ${ active ? style.badgeClassName : 'border-white/10 bg-white/[0.03] text-[var(--color-pib-text-muted)] hover:bg-white/[0.06]' }`}
          >
            {row.label}
...
export function ContextDock({ model, open, onClose, compact = false, activeArtifactId, onArtifactActivate, onAction, actionError, actionReceipt, pendingActionId, execution, mode = 'single', onModeChange, canvasWidth = 520, onCanvasWidthChange, secondaryContext, secondaryOptions = [], onSecondaryChange, secondaryRefreshRevision = 0, previewRefreshRevision = 0, workbenchFolder }: { model: ChatContextReadModel; open: boolean; onClose: () => void; compact?: boolean; activeArtifactId?: string; onArtifactActivate?: (artifact: ChatArtifactSummary) => void; onAction?: (action: ChatContextAction, context?: ChatContextOption) => void; actionError?: string | null; actionReceipt?: ChatContextActionReceipt | null; pendingActionId?: string; execution?: RuntimeExecution; mode?: 'single' | 'dual'; onModeChange?: (mode: 'single' | 'dual') => void; canvasWidth?: number; onCanvasWidthChange?: (width: number) => void; secondaryContext?: ChatContextOption; secondaryOptions?: ChatContextOption[]; onSecondaryChange?: (context: ChatContextOption) => void; secondaryRefreshRevision?: number; previewRefreshRevision?: number; workbenchFolder?: { conversationId: string; path: string } }) {
  ...
  return <>{sheet && <div aria-hidden="true" onClick={onClose} className="fixed inset-0 z-30 bg-black/55" />}<aside ref={dialogRef} tabIndex={-1} role="dialog" aria-modal={sheet ? 'true' : 'false'} aria-label={`${visibleContext.label} context`} data-presentation={sheet ? 'sheet' : dual ? 'dual' : 'canvas'} style={!sheet ? { width: `${canvasWidth}px` } : undefined} className={sheet ? 'fixed inset-0 z-40 flex flex-col overflow-hidden border border-[var(--color-card-border)] bg-[var(--color-surface,#151515)] pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)] ' : 'absolute inset-y-0 right-0 z-40 flex max-w-[min(960px,85%)] flex-col overflow-hidden border border-[var(--color-card-border)] bg-[var(--color-surface,#151515)] '}>
    ...
    <div id="context-dock-scroll-body" data-testid="context-dock-scroll-body" className={`min-h-0 flex-1 overflow-y-auto px-3 pt-3 pb-[max(.75rem,env(safe-area-inset-bottom))] ${dual ? 'grid content-start gap-3' : 'space-y-4'}`}>
      ...
      {visibleModel?.preview?.kind === 'summary' && !DOCK_PREVIEW_KINDS.has(visibleModel.context.kind) && (visibleModel.preview.text || visibleModel.preview.status) && <section aria-label="Context overview" className="rounded-[6px] border border-[var(--color-card-border)] bg-white/[0.025] p-3"><div className="flex items-center justify-between gap-2"><h3 className="text-[10px] font-label uppercase tracking-[0.18em] text-[var(--color-pib-text-muted)]">Overview</h3>{visibleModel.preview.status && <span className="rounded-[4px] border border-primary/25 bg-primary/10 px-2 py-0.5 text-[10px] font-medium text-primary">{visibleModel.preview.status}</span>}</div>{visibleModel.preview.text && <p className="mt-2 text-xs leading-relaxed text-[var(--color-pib-text)]">{visibleModel.preview.text}</p>}</section>}
