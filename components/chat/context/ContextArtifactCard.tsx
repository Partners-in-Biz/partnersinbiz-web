...
export function ContextArtifactCard({ artifact, selected = false, onActivate, onAction, pendingActionId }: { artifact: ChatArtifactSummary; selected?: boolean; onActivate?: (artifact: ChatArtifactSummary) => void; onAction?: (action: ChatContextAction) => void; pendingActionId?: string }) {
  ...
  return <article data-selected={selected || undefined} className="overflow-hidden rounded-lg border border-white/10 bg-black/15">
    <Preview artifact={artifact} />
    <button type="button" aria-label={`Inspect ${artifact.title}`} aria-current={selected ? 'true' : undefined} onClick={() => onActivate?.(artifact)} className="flex w-full items-center gap-2 px-3 py-2.5 text-left hover:bg-white/[0.035]">
    ...
    {artifact.actions.length > 0 && <div className="flex flex-wrap gap-2 border-t border-white/[0.06] px-3 py-2">{artifact.actions.map((action) => action.href && !action.method ? <a key={action.id} href={action.href} className="text-xs text-primary">{action.label}</a> : <button key={action.id} type="button" disabled={pendingActionId === action.id} onClick={() => onAction?.(action)} className="rounded-md bg-primary px-2.5 py-1 text-xs text-on-primary disabled:opacity-50">{action.label}</button>)}</div>}
    {artifact.provenance && <div className="border-t border-white/[0.06] px-3 py-2">
      <button type="button" aria-expanded={details} aria-label={`${details ? 'Hide' : 'Show'} provenance for ${artifact.title}`} onClick={() => setDetails((value) => !value)} className="text-[10px] text-on-surface-variant hover:text-on-surface">{details ? 'Hide' : 'Show'} provenance</button>
