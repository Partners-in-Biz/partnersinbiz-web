...
export function ContextSelector({ options, value, onChange }: { options: ChatContextOption[]; value: ChatContextReference; onChange: (value: ChatContextReference) => void }) {
  ...
  const encoded = chatContextReferenceKey(value)
  return <select aria-label="Active context" value={encoded} onChange={(event) => {
    const option = options.find((item) => chatContextReferenceKey(item) === event.target.value)
    if (option) onChange({ kind: option.kind, id: option.id, ...(option.projectId ? { projectId: option.projectId } : {}) })
    ...
    {options.map((option) => <option key={chatContextReferenceKey(option)} value={chatContextReferenceKey(option)}>{option.label}</option>)}
  </select>
}
