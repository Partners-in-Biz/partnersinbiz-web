    ...
    <section className="grid min-h-[620px] min-w-0 lg:grid-cols-[minmax(320px,0.72fr)_minmax(0,1.28fr)]">
        ...
        <div className="max-h-[540px] overflow-auto">
          {loading ? <p className="p-4 text-xs text-[var(--color-pib-text-muted)]">Loading replies…</p> : items.length === 0 ? <p className="p-4 text-xs text-[var(--color-pib-text-muted)]">No replies match these filters.</p> : items.map((item) => (
            <button key={item.id} onClick={() => setSelectedId(item.id)} className={`block w-full border-b border-[var(--color-card-border)] p-3 text-left ${selected?.id === item.id ? 'bg-primary/[0.06]' : 'hover:bg-white/[0.025]'}`}>
              <div className="flex items-center gap-2"><span className="rounded bg-primary/10 px-1.5 py-0.5 text-[10px] capitalize text-primary">{label(item.classification)}</span><span className={`ml-auto text-[10px] font-medium uppercase ${item.slaState === 'missed' ? 'text-error' : 'text-[var(--color-pib-text-muted)]'}`}>{item.slaState === 'missed' ? 'SLA missed' : item.slaState}</span></div>
      ...
      <div className="min-w-0 p-4">
        {!selected ? <p className="text-sm text-[var(--color-pib-text-muted)]">Select a reply to inspect its CRM handoff.</p> : <div className="space-y-4">
          <div><div className="flex flex-wrap items-center gap-2"><h2 className="text-base font-semibold">{selected.subject || '(no subject)'}</h2>{selected.corrected ? <span className="text-[10px] text-[var(--color-pib-text-muted)]">Human-corrected</span> : null}</div><p className="mt-1 text-xs text-[var(--color-pib-text-muted)]">From {selected.fromEmail}{selected.receivedAt ? ` · ${new Date(selected.receivedAt).toLocaleString()}` : ''}</p></div>
