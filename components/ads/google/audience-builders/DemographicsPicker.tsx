'use client'
// components/ads/google/audience-builders/DemographicsPicker.tsx
// Google Detailed Demographics audience picker — Sub-3a Phase 5 Batch 3 F
...
export function DemographicsPicker({ orgId }: Props) {
  ...
  useEffect(() => {
    fetch('/api/v1/ads/google/audiences/browse?type=DETAILED_DEMOGRAPHICS', {
  ...
  if (state === 'loading') {
    return <p className="text-sm text-white/40">Loading demographic segments…</p>
  }

  if (state === 'done') {
    return (
      <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-4 text-sm">
        <p className="font-medium text-emerald-300">Demographic audience saved</p>
        <p className="mt-1 text-xs text-white/60">
          This demographic segment is now available for admin-built Google ad sets.
        </p>
    ...
    <div className="space-y-5">
      <label className={labelCls}>
        ...
        <input
          ...
          placeholder="e.g. Parents — Detailed Demographics"
      ...
      <div>
        <label className={labelCls}>
          Search demographic segments
          <input
            className={inputCls}
            value={filter}
            onChange={(e) => setFilter(e.target.value)}
            aria-label="Search demographic segments"
        ...
        <div className="mt-2 max-h-64 overflow-y-auto rounded border border-white/10">
          {filtered.length === 0 ? (
            <p className="px-4 py-3 text-sm text-white/40">No segments match the admin filter.</p>
          ) : (
            filtered.map((a) => (
              <label
                key={a.resourceName}
