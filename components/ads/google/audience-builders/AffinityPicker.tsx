'use client'
// components/ads/google/audience-builders/AffinityPicker.tsx
// Google Affinity audience picker — Sub-3a Phase 5 Batch 3 F
...
export function AffinityPicker({ orgId }: Props) {
  ...
  useEffect(() => {
    fetch('/api/v1/ads/google/audiences/browse?type=AFFINITY', {
  ...
  if (state === 'loading') {
    return <p className="text-sm text-white/40">Loading affinity audiences…</p>
  }
    ...
    <div className="space-y-5">
      <label className={labelCls}>
        ...
        <input
          ...
          placeholder="e.g. Sports Fans — Affinity"
      ...
      <div>
        <label className={labelCls}>
          Search affinity categories
          <input
            className={inputCls}
            value={filter}
            onChange={(e) => setFilter(e.target.value)}
            aria-label="Search affinity categories"
