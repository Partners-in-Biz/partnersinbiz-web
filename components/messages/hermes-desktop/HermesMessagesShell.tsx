      ...
      <section data-testid="hermes-messages-shell-body" className="min-h-0 min-w-0 flex-1 overflow-hidden p-2">
        <div className={`flex h-full min-h-0 min-w-0 ${direction === 'row' ? 'flex-row' : 'flex-col'}`}>
          {panes.map((pane, paneIndex) => {
            ...
            return (
              <div key={pane.id} data-testid={`messages-workspace-pane-${pane.id}`} style={style} onPointerDown={() => setFocusedPaneId(pane.id)} className={`flex min-h-0 min-w-0 flex-col overflow-hidden rounded-xl border ${focusedPaneId === pane.id ? 'border-primary/35' : 'border-[var(--color-card-border)]'} bg-black/[0.035] ${pane.id !== focusedPaneId ? 'max-md:hidden' : ''}`}>
          ...
          {panes.length > 1 && <button type="button" aria-label="Resize workspace panes" style={{ order: 1 }} onPointerDown={startResize} onPointerMove={continueResize} onPointerUp={finishResize} onPointerCancel={finishResize} className={`z-10 shrink-0 touch-none rounded-full bg-transparent hover:bg-primary/20 focus-visible:bg-primary/20 ${direction === 'row' ? '-mx-0.5 min-h-11 min-w-11 cursor-col-resize xl:w-2 xl:min-w-0' : '-my-0.5 min-h-11 min-w-11 cursor-row-resize xl:h-2 xl:min-h-0'}`} />}
        </div>
      </section>
    </div>
  )
