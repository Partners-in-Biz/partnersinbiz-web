...
/**
 * Shared WebGL2 neural-field atmosphere (Dense Cinematic kit).
 * Pure WebGL — no three.js. Disabled under prefers-reduced-motion / no WebGL2.
 */
...
void main() {
  ...
  vec2 r = vec2(fbm(p * 2.1 + 4.0 * q + vec2(1.7, 9.2) + t * 0.35), fbm(p * 2.1 + 4.0 * q + vec2(8.3, 2.8) - t * 0.25));
...
export function NeuralField({ className = '', intensity = 1, testId = 'pib-neural-field' }: NeuralFieldProps) {
  ...
  return (
    <canvas
      ...
      aria-hidden="true"
      data-testid={testId}
      className={cn('pib-neural-field messages-neural-field', className)}
    />
  )
}
