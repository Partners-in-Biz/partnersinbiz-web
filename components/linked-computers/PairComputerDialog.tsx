...
export function PairComputerDialog({ onClose }: { onClose(): void }) {
  ...
  return <AccessibleDialog label="Pair a computer" onClose={onClose} className="w-full max-w-lg rounded-2xl border border-[var(--color-pib-line)] bg-[var(--color-card)] p-6 shadow-xl">
      <div className="flex items-start justify-between gap-4">
        <div><h2 id="pair-title" className="text-lg font-semibold">Pair a computer</h2><p className="mt-1 text-sm text-[var(--color-pib-text-muted)]">Create a one-time code, then enter it in the Partners in Biz runtime on your computer.</p></div>
      ...
      {pairing ? <div className="mt-5 rounded-xl border border-primary/30 bg-primary/10 p-5 text-center">
        ...
        <p className="mt-2 text-left text-xs text-[var(--color-pib-text-muted)]">Signed installers are delivered in Stage 6. Until then this handoff works only with an authorised development runtime already installed.</p>
