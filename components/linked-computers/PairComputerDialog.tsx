...
export function PairComputerDialog({
  ...
  return <AccessibleDialog label="Link a computer or VPS" onClose={onClose} className="w-full max-w-lg rounded-2xl border border-[var(--color-pib-line)] bg-[var(--color-card)] p-6 shadow-xl">
      ...
      {!guidedSetupReady && <div role="status" className="mt-4 rounded-xl border border-amber-400/25 bg-amber-400/10 p-4 text-sm text-amber-100"><p className="font-medium">{platformLabel} secure installer awaiting release</p><p className="mt-1 text-xs text-amber-100/75">Pairing stays disabled until the signed runtime bundle is published. This prevents an incomplete or unsigned setup from being handed to a customer.</p></div>}
