...
export function LinkedComputersWorkspace() {
  ...
  return <div className="mx-auto max-w-5xl p-4 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><p className="pib-eyebrow">Settings</p><h1 className="pib-page-title mt-2">Linked Computers</h1><p className="mt-2 max-w-2xl text-sm text-[var(--color-pib-text-muted)]">Run Workspace work on a computer you control. Physical folders stay private on that computer.</p></div>
      ...
      {devices.map((device) => {
        ...
        return <article key={device.deviceId} aria-label={device.label} className="rounded-2xl border border-[var(--color-pib-line)] bg-[var(--color-card)] p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div><div className="flex items-center gap-2"><h2 className="font-semibold">{device.label}</h2><span className={`rounded-full px-2 py-0.5 text-xs ${online ? 'bg-emerald-400/10 text-emerald-300' : 'bg-white/5 text-[var(--color-pib-text-muted)]'}`}>{online ? 'Online' : 'Offline'}</span></div><p className="mt-1 text-xs text-[var(--color-pib-text-muted)]">{device.platform === 'macos' ? 'macOS' : 'Windows'} · {device.architecture} · Version {device.runtimeVersion}</p></div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div><p className="text-xs font-semibold">Organisation access</p>{device.grants?.length ? device.grants.map(g => <p key={g.orgId} className="mt-1 text-sm text-[var(--color-pib-text-muted)]">{g.orgLabel ?? g.orgId} · {g.status}</p>) : <p className="mt-1 text-sm text-[var(--color-pib-text-muted)]">No organisation granted</p>}</div>
            <div><p className="text-xs font-semibold">Workspace mappings</p>{device.mappings?.length ? device.mappings.map(m => <p key={m.mappingId} className="mt-1 text-sm text-[var(--color-pib-text-muted)]">{m.label} · {m.status === 'active' ? 'Mapped' : m.status}</p>) : <p className="mt-1 text-sm text-[var(--color-pib-text-muted)]">No Workspace mapped</p>}</div>
          </div>
        </article>
      })}
      {!error && devices.length === 0 && <p className="rounded-2xl border border-dashed border-[var(--color-pib-line)] p-8 text-center text-sm text-[var(--color-pib-text-muted)]">No computers linked yet.</p>}
    </div>
