...
const INPUT =
  'w-full px-3 py-2 rounded-lg border border-[var(--color-card-border)] bg-[var(--color-pib-surface-soft)] text-[var(--color-pib-text)] text-sm'
const CARD =
  ...
  if (loadError) {
    return (
      <section className="rounded-[var(--radius-card)] border border-amber-500/25 bg-[color-mix(in_srgb,var(--st-warning)_12%,transparent)]/[0.07] p-5">
        ...
        <button type="button" onClick={loadSource} className="mt-3 h-8 rounded-md border border-[var(--color-card-border)] px-2.5 text-xs text-[var(--color-pib-text-muted)] hover:bg-white/[0.05] hover:text-[var(--color-pib-text)]">
          Retry
        </button>
      </section>
    )
        ...
        <div className="flex flex-wrap gap-2">
          {FIELD_PRESETS.map((p) => (
            <button
              ...
              className="px-2.5 py-1 rounded-md text-xs bg-white/[0.04] hover:bg-white/[0.08] text-[var(--color-pib-text)] border border-[var(--color-card-border)] transition-colors disabled:opacity-40"
            >
              + {p.label}
            </button>
          ))}
        ...
        {fields.length > 0 && (
          <div className="space-y-3">
            {fields.map((f, idx) => (
              <div
                key={idx}
                className="rounded-lg border border-[var(--color-card-border)] bg-[var(--color-pib-surface-soft)] p-3 space-y-2"
              >
            ...
            {deliveries.length === 0 ? (
              ...
              <div className="space-y-2">
                {deliveries.map((d) => (
                  <div
                    key={d.id}
                    className="rounded-lg border border-[var(--color-card-border)] bg-[var(--color-pib-surface-soft)] p-3"
                  >
