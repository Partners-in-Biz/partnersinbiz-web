...
interface CaptureSourcesWorkspaceProps {
  orgId?: string
  orgName?: string
  importHref?: string
          ...
          <div>
            ...
            {sequences.length === 0 ? (
              <p className="text-sm text-[var(--color-pib-text-muted)]">
                ...
                <Link href={sequenceNewHref} className="text-[var(--color-pib-accent)] hover:underline">
                  Create a sequence
                </Link>
              </p>
            ) : (
...
export function CaptureSourcesWorkspace({
  orgId,
  orgName,
  ...
  const sourcesEndpoint = useMemo(
    () => scopedUrl('/api/v1/crm/capture-sources', scopedOrgId),
    [scopedOrgId],
  )
  ...
  const sequencesEndpoint = useMemo(
    () => scopedUrl('/api/v1/crm/sequences', scopedOrgId),
    [scopedOrgId],
  )
  const sourceEndpoint = useCallback(
    (sourceId: string) => scopedUrl(`/api/v1/crm/capture-sources/${sourceId}`, scopedOrgId),
    [scopedOrgId],
  )
