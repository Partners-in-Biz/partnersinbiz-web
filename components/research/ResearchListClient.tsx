  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="sc-tiny">{orgName || (mode === 'portal' ? 'Research' : 'Intelligence')}</p>
      ...
      {(mode === 'admin' || canCreateResearch) && (
        <section className="bento-card grid gap-3 md:grid-cols-[1fr_auto]">
          ...
          <button type="button" onClick={createResearch} disabled={!newTitle.trim() || creating} className="btn-pib-accent disabled:opacity-50">
            <Icon name={creating ? 'progress_activity' : 'add'} />
            New Research
          </button>
        </section>
      )}
      ...
      <section className="bento-card grid gap-3 md:grid-cols-5">
        {mode === 'admin' && orgs.length > 0 && (
          <select value={activeOrgId} onChange={(event) => setActiveOrgId(event.target.value)} className="pib-select" aria-label="Input">
            {orgs.map((org) => <option key={org.id} value={org.id}>{org.name}</option>)}
          </select>
        )}
        <select value={kind} onChange={(event) => setKind(event.target.value as ResearchKind | 'all')} className="pib-select" aria-label="Input">
          <option value="all">All kinds</option>
          {RESEARCH_KINDS.map((value) => <option key={value} value={value}>{label(value)}</option>)}
        </select>
        <select value={status} onChange={(event) => setStatus(event.target.value as ResearchStatus | 'all')} className="pib-select" aria-label="Input">
          <option value="all">All statuses</option>
          {RESEARCH_STATUSES.map((value) => <option key={value} value={value}>{label(value)}</option>)}
        </select>
        {mode === 'admin' && (
          <select value={visibility} onChange={(event) => setVisibility(event.target.value as ResearchVisibility | 'all')} className="pib-select" aria-label="Input">
            <option value="all">All visibility</option>
            {RESEARCH_VISIBILITIES.map((value) => <option key={value} value={value}>{label(value)}</option>)}
          </select>
        )}
        <input value={q} onChange={(event) => setQ(event.target.value)} placeholder="Search research" className="pib-input"  aria-label="Search research"/>
      </section>
      ...
      {loading ? (
        ...
        <div className="bento-card p-10 text-center">
          ...
          <h2 className="mt-4 text-2xl">No research matches this view.</h2>
