    ...
    <BlockFrame block={block} index={index}>
      <div className="pib-card overflow-hidden !p-0" data-showcase-block={block.type}>
        <div className="border-b border-[var(--color-card-border)] bg-[var(--color-card)]/80 px-5 py-5 sm:px-6">
          ...
          {title && <h2 className="mt-2 text-2xl font-medium text-[var(--doc-text)] md:text-4xl">{title}</h2>}
...
function EmptyShowcaseState({ label }: { label: string }) {
  return <p className="rounded-md border border-dashed border-[var(--color-card-border)] p-5 text-sm text-[var(--doc-muted)]">No {label} supplied yet.</p>
}
      ...
      {stages.length === 0 ? <EmptyShowcaseState label="funnel stages" /> : (
        <ol aria-label={content?.headline || block.title || 'Funnel stages'} className="grid gap-3 md:grid-cols-2">
          {stages.map((stage, i) => {
            ...
            return (
              <li key={stage.id || `${stage.label}-${i}`} className="rounded-md border border-[var(--color-card-border)] bg-[var(--color-card)] p-4 transition-colors motion-safe:duration-200 hover:border-[var(--doc-accent)]/50">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs uppercase tracking-[0.18em] text-[var(--doc-muted)]">Stage {i + 1}</p>
                    <h3 className="mt-1 text-lg font-medium text-[var(--doc-text)]">{stage.label}</h3>
                  </div>
                  {typeof stage.value === 'number' && <span className="rounded-md bg-[var(--doc-accent-soft)] px-3 py-1 text-sm font-medium text-[var(--doc-accent)]">{stage.value}</span>}
                </div>
                {stage.description && <p className="mt-3 text-sm leading-6 text-[var(--doc-muted)]">{stage.description}</p>}
                <div className="mt-4 h-2 rounded-md bg-[var(--doc-border)]" aria-hidden="true">
                  <div className="h-full rounded-md bg-[var(--doc-accent)] motion-safe:transition-[width] motion-safe:duration-700" style={{ width: `${width}%` }} />
                </div>
      ...
      {axes.length === 0 ? <EmptyShowcaseState label="radar axes" /> : (
        <div className="grid gap-6 lg:grid-cols-[minmax(240px,360px)_1fr] lg:items-center">
          ...
          <ul className="grid gap-3 sm:grid-cols-2">
            {axes.map((axis) => (
              <li key={axis.id} className="rounded-md border border-[var(--color-card-border)] bg-[var(--color-card)] p-4">
...
export function QuadrantMatrixBlock({ block, index }: { block: DocumentBlock; index: number }) {
  ...
  return (
    <ShowcaseShell block={block} index={index} eyebrow={content?.eyebrow} headline={content?.headline} description={content?.description}>
      <div role="img" aria-label={content?.headline || block.title || 'Quadrant matrix'} className="relative min-h-[320px] rounded-md border border-[var(--color-card-border)] bg-[var(--color-card)] p-5">
        ...
        {items.map((item) => (
          <div key={item.id} className="absolute max-w-[11rem] -translate-x-1/2 -translate-y-1/2 rounded-md border border-[var(--doc-accent)]/40 bg-[var(--doc-accent-soft)] px-3 py-2 text-sm motion-safe:transition-transform motion-safe:duration-200 hover:scale-105" style={{ left: `${clamp(item.x)}%`, top: `${100 - clamp(item.y)}%` }}>
    ...
    <ShowcaseShell block={block} index={index} eyebrow={content?.eyebrow} headline={content?.headline} description={content?.description}>
      {pairs.length === 0 ? <EmptyShowcaseState label="before/after pairs" /> : (
        <div className="grid gap-4 md:grid-cols-2">
          {pairs.map((pair) => (
            <article key={pair.id} className="rounded-md border border-[var(--color-card-border)] bg-[var(--color-card)] p-5">
              <h3 className="text-lg font-medium text-[var(--doc-text)]">{pair.label}</h3>
              <dl className="mt-4 grid gap-3">
                <div className="rounded-md bg-red-500/10 p-4"><dt className="text-xs uppercase tracking-wider text-red-200">Before</dt><dd className="mt-1 text-sm text-[var(--doc-text)]">{pair.before}</dd></div>
                <div className="rounded-md bg-emerald-500/10 p-4"><dt className="text-xs uppercase tracking-wider text-emerald-200">After</dt><dd className="mt-1 text-sm text-[var(--doc-text)]">{pair.after}</dd></div>
              </dl>
      ...
      {items.length === 0 ? <EmptyShowcaseState label="roadmap items" /> : (
        <div className="space-y-4" role="list" aria-label={content?.headline || block.title || 'Roadmap timeline'}>
          {items.map((item) => {
            ...
            return (
              <div key={item.id} role="listitem" className="rounded-md border border-[var(--color-card-border)] bg-[var(--color-card)] p-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div><p className="text-xs uppercase tracking-wider text-[var(--doc-muted)]">{item.lane || 'Timeline'}</p><h3 className="font-medium text-[var(--doc-text)]">{item.label}</h3></div>
                ...
                <div className="mt-4 h-3 rounded-md bg-[var(--doc-border)]" aria-hidden="true"><div className="h-full rounded-md bg-[var(--doc-accent)] motion-safe:transition-all motion-safe:duration-700" style={{ marginLeft: `${clamp(offset)}%`, width: `${clamp(width, 8, 100)}%` }} /></div>
                {item.owner && <p className="mt-2 text-xs text-[var(--doc-muted)]">Owner: {item.owner}</p>}
              </div>
            )
          })}
          {content?.milestones?.length ? <div className="flex flex-wrap gap-2 pt-2">{content.milestones.map((m) => <span key={m.id} className="rounded-md border border-[var(--doc-accent)]/40 px-3 py-1 text-xs text-[var(--doc-accent)]">{m.label}: {formatDate(m.date)}</span>)}</div> : null}
        </div>
      )}
    </ShowcaseShell>
  )
...
export function LogoTestimonialProofBlock({ block, index }: { block: DocumentBlock; index: number }) {
  ...
  return (
    <ShowcaseShell block={block} index={index} eyebrow={content?.eyebrow} headline={content?.headline} description={content?.description}>
      <div className="grid gap-4 md:grid-cols-2">
        {proof.map((item) => (
          <figure key={item.id} className="rounded-md border border-[var(--color-card-border)] bg-[var(--color-card)] p-5">
            {item.logoUrl ? <img src={item.logoUrl} alt={item.organisationName || 'Client logo'} className="mb-4 max-h-12 max-w-40 object-contain" /> : <div className="mb-4 inline-flex rounded-md bg-[var(--doc-accent-soft)] px-3 py-1 text-xs uppercase tracking-wider text-[var(--doc-accent)]">{item.kind}</div>}
            {item.metricValue && <p className="text-3xl font-medium text-[var(--doc-accent)]">{item.metricValue}</p>}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {cards.map((card) => (
          <article key={card.id} className="pib-card pib-card-hover !p-5">
            {card.imageUrl && <img src={card.imageUrl} alt="" className="mb-4 aspect-video w-full rounded-md object-cover" />}
            <p className="text-3xl font-medium text-[var(--doc-accent)]">{card.result}</p>
            <h3 className="mt-3 text-lg font-medium text-[var(--doc-text)]">{card.title}</h3>
    ...
    <ShowcaseShell block={block} index={index} eyebrow={content?.eyebrow} headline={content?.headline} description={content?.description}>
      <div className="overflow-x-auto rounded-md border border-[var(--color-card-border)]">
        <table aria-label={content?.headline || block.title || 'Weighted decision matrix'} className="min-w-full divide-y divide-[var(--color-card-border)] bg-[var(--color-card)] text-sm">
          <thead><tr><th scope="col" className="px-4 py-3 text-left font-medium">Option</th>{criteria.map((criterion) => <th key={criterion.id} scope="col" className="px-4 py-3 text-left font-medium">{criterion.label} <span className="text-[var(--doc-muted)]">({Math.round(criterion.weight * 100)}%)</span></th>)}<th scope="col" className="px-4 py-3 text-left font-medium">Weighted score</th></tr></thead>
          <tbody className="divide-y divide-[var(--color-card-border)]">
            {options.map((option) => {
              const total = totals.find((t) => t.id === option.id)?.total ?? 0
              return <tr key={option.id} className={option.recommended ? 'bg-[var(--doc-accent-soft)]' : undefined}><th scope="row" className="px-4 py-3 text-left font-medium text-[var(--doc-text)]">{option.label}{option.recommended && <span className="ml-2 rounded-md bg-[var(--doc-accent)] px-2 py-0.5 text-[10px] uppercase text-black">Recommended</span>}{option.summary && <span className="mt-1 block text-xs font-normal text-[var(--doc-muted)]">{option.summary}</span>}</th>{criteria.map((criterion) => <td key={criterion.id} className="px-4 py-3 text-[var(--doc-muted)]">{option.scores?.[criterion.id] ?? 0}</td>)}<td className="px-4 py-3 font-medium text-[var(--doc-accent)]">{total.toFixed(1)}</td></tr>
            })}
          </tbody>
        </table>
      </div>
