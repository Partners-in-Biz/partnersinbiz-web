...
interface MailboxWorkspaceProps {
  surface: MailboxSurface
  showCloseAction?: boolean
  onClose?: () => void
}

const MAILBOX_CONFIG: Record<MailboxSurface, {
  eyebrow: string
  title: string
...
export function MailboxWorkspace({ surface, showCloseAction = false, onClose }: MailboxWorkspaceProps) {
  ...
  async function submitCompose(action: 'send' | 'draft') {
    setError(null)
    const res = await fetch(config.messagesEndpoint, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...compose,
  ...
  async function updateMessage(id: string, patch: Record<string, unknown>) {
    await fetch(`${config.messagesEndpoint}/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function deleteMessage(id: string) {
    await fetch(`${config.messagesEndpoint}/${id}`, { method: 'DELETE' })
    await loadMessages()
  }
  ...
  async function syncAccount(account: MailboxAccountSafe) {
    ...
    try {
      const res = await fetch(`${config.accountsEndpoint}/${account.id}/sync`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          mode: account.lastSyncAt ? 'incremental' : 'backfill',
          maxResults: account.lastSyncAt ? 50 : 100,
        }),
      })
      const body = await res.json()
                  ...
                  {account.provider === 'google' && account.status === 'connected' ? (
                    <div className="mt-2">
                      <button
                        ...
                        className="btn-pib-secondary !px-2 !py-1 text-[10px]"
                      >
