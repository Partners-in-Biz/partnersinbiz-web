...
export function PortalViewSwitch({ orgId, collapsed = false, compact = false, iconOnly = false }: PortalViewSwitchProps) {
  ...
  const portalOpenLabel = 'Open client portal as admin'
      ...
      if (!res.ok) {
        throw new Error(
          typeof body?.error === 'string'
            ? body.error
            : 'Add this admin as a team member before opening the client portal.',
        )
      }
      router.push('/portal/dashboard')
      router.refresh()
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Could not open the client portal.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className={collapsed ? 'px-2 py-2' : 'px-3 py-3'}>
      ...
      >
        <span className="material-symbols-outlined text-[20px] shrink-0 opacity-70">open_in_new</span>
