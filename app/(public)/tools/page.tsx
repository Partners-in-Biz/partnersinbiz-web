          ...
          <Reveal delay={320}>
            <div className="mt-12 grid max-w-4xl gap-3 sm:grid-cols-3">
              {[
                ...
                <div key={label} className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface)]/70 px-5 py-4">
                  <p className="text-3xl text-[var(--color-pib-text)]">{value}</p>
            ...
            {PUBLIC_TOOLS.map((tool, index) => (
              <Reveal key={tool.slug} delay={index * 60}>
                <Link href={`/tools/${tool.slug}`} className="bento-card group relative flex h-full flex-col gap-6 overflow-hidden p-8 transition duration-300 hover:-translate-y-1 hover:border-[var(--color-pib-accent)]/50 hover:shadow-[0_24px_90px_rgba(50,255,214,0.14)]">
                  <div className="absolute inset-x-6 top-0 h-px bg-[var(--color-pib-accent)]/50 opacity-0 transition group-hover:opacity-100" />
                  <div className="flex items-start justify-between gap-4">
                    <Icon name={tool.icon} />
                    <span className="pill text-xs">{tool.category}</span>
                  </div>
                  <div className="flex-1">
                    <h2 className="text-2xl leading-tight text-[var(--color-pib-text)]">
                      {tool.title}
                    </h2>
                  ...
                  <div className="grid gap-3 rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-bg)]/40 p-4">
                    <div>
        ...
        <div className="container-pib">
          <div className="bento-card mb-8 grid gap-6 overflow-hidden p-8 lg:grid-cols-[1fr_0.8fr]">
            <div>
              ...
              <h2 className="text-3xl text-[var(--color-pib-text)] md:text-5xl">Calculators for commercial judgement. Checkers for technical proof. CTAs for real work.</h2>
            </div>
            <div className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-bg)]/50 p-5 text-sm leading-relaxed text-[var(--color-pib-text-muted)]">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {[
              ...
              <div key={title} className="bento-card p-7">
                <h3 className="text-2xl text-[var(--color-pib-text)]">{title}</h3>
