...
export async function generateMetadata({
  ...
  const title = `${study.client}: ${study.headline}`
...
export default async function CaseStudyPage({
  ...
  return (
    <Article>
      ...
      <ArticleHead
        kicker={
          <>
            ...
            {' / '}
            {study.client}. {study.industry}. {study.year}.
          </>
        }
        title={study.headline}
        lede={study.summary}
      ...
      <ArticleRow title="The brief" aside={<Quote quote={content.pullQuote} by={`${content.contact.name}, ${content.contact.role}`} />}>
        <p>{content.brief}</p>
      </ArticleRow>

      <ArticleRow title="What we saw" flip>
        <p>{content.insight}</p>
      </ArticleRow>
      ...
      >
        <p>{content.build}</p>
        <p className="sc-tiny">Built with {study.stack.join(', ')}.</p>
      </ArticleRow>

      <ArticleRow
        title="What changed"
        flip
        aside={<Quote quote={content.outcomeQuote.quote} by={`${content.outcomeQuote.author}, ${content.outcomeQuote.role}`} />}
      >
        <ArticleList items={study.metrics.map((m) => `${m.value} ${m.label}`)} />
        <p className="sc-tiny">Shipped</p>
        <ArticleList items={content.shipped} />
      </ArticleRow>

      <ArticleRow
        title="Next case"
        aside={<Plate src={next.cover} alt={`${next.client}: ${next.headline}`} />}
      >
        <p>
          <Link href={next.href} prefetch={false} className="sc-cta">
            {next.headline}
          </Link>
        </p>
        <p className="sc-tiny">
        ...
        <CtaSentence lead="Want one of these with your name on it?" />
      </ArticleRow>
    </Article>
  )
}
