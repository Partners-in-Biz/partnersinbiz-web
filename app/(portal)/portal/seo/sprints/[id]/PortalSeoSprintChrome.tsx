...
export function PortalSeoSprintChrome({
  ...
  return (
    <div className="space-y-4" data-module-accent="green">
      <header className="st-panel !p-0 overflow-hidden">
        ...
        <div className="p-4 md:p-5">
          ...
          <div className="mt-3 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-4 lg:items-end">
            <div>
              <p className="sc-tiny">SEO sprint</p>
              ...
              {accessMode === 'projected' ? (
                <span className="mt-2 inline-flex items-center rounded bg-[var(--color-accent-v2)]/15 px-2.5 py-1 text-[11px] text-[var(--color-pib-text-muted)]">
          ...
          <div className="mt-4">
            ...
            <div className="h-1.5 rounded bg-[var(--color-pib-line)] overflow-hidden">
              <div className="h-full rounded bg-[var(--color-pib-accent)]" style={{ width: `${progress}%` }} />
            </div>
          </div>
        </div>
      </header>

      <nav className="st-panel !p-1.5 flex gap-1 overflow-x-auto">
        {TABS.map((tab) => (
          <Link
            key={tab.key}
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-2.5">
      ...
      <p className="text-lg tabular-nums mt-0.5">{value}</p>
    </div>
  )
}
