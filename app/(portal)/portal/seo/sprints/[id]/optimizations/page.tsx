    ...
    <div className="space-y-5">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="text-2xl text-[var(--color-pib-text)]">Optimizations</h2>
        </div>
      ...
      {opts.length === 0 ? (
        <div className="st-panel py-10 text-center text-sm text-[var(--color-pib-text-muted)]">
        ...
        <ul className="space-y-3">
          {opts.map((o) => (
            <li key={o.id} className="st-panel space-y-3">
              ...
              <p className="border border-[var(--color-pib-line)] bg-white/[0.02] p-3 text-xs text-[var(--color-pib-text-muted)]">{o.proposedAction}</p>
              {o.outcomeDelta && (
                <p className="text-xs">
