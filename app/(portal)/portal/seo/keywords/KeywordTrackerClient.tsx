...
function PositionChangePill({ delta }: { delta: number | null }) {
  ...
  return (
    <span className={`inline-flex items-center gap-0.5 text-[11px] ${improved ? 'text-emerald-300' : 'text-red-300'}`}>
      <Icon name={improved ? 'arrow_upward' : 'arrow_downward'} />
...
export function KeywordTrackerClient({
  ...
  const actionSlot = (
    <div className="flex flex-wrap items-center gap-2">
      ...
      >
        <span className={`inline-flex ${importing ? 'animate-spin' : ''}`}>
          <Icon name={importing ? 'autorenew' : 'cloud_download'} />
        </span>
        {importing ? 'Importing…' : 'Import from Search Console'}
      </button>
      <button name="keywordtrackerclient-action-60" onClick={handleExportCsv} disabled={initialKeywords.length === 0} className="pib-btn-secondary text-sm disabled:opacity-40">
        <Icon name="download" />
        CSV
      </button>
    </div>
  ...
  return (
    <div className="space-y-4" data-module-accent="green">
        ...
        <section className="pib-card-section">
          ...
          <form onSubmit={handleAdd} className="flex flex-wrap items-end gap-3 p-4">
            ...
            <div className="flex flex-col gap-1 w-36">
              ...
              <select name="keywordtrackerclient-select-62" value={addIntent} onChange={(e) => setAddIntent(e.target.value as IntentBucket)} className="pib-select">
                <option value="problem">Problem</option>
                <option value="solution">Solution</option>
                <option value="brand">Brand</option>
              </select>
            </div>
            <button name="keywordtrackerclient-action-63" type="submit" disabled={adding || !addKeyword.trim()} className="pib-btn-primary text-sm self-end disabled:opacity-50">
      ...
      {/* Filters */}
      <div className="flex flex-wrap items-center gap-2">
        <span className="sc-tiny !text-[10px] mr-1">Position filter</span>
        {(['all', 'top3', 'top10', '11-50', '51-100', 'not-ranking'] as PositionRange[]).map((r) => (
          <button name="keywordtrackerclient-action-64"
            key={r}
 ...
 : 'border-[var(--color-pib-line)] text-[var(--color-pib-text-muted)] hover:text-[var(--color-pib-text)]'
      ...
      {initialKeywords.length === 0 ? (
        <div className="st-panel p-10 text-center">
          <Icon name="key_off" />
          <h3 className="font-headline text-lg mt-3">No keywords tracked yet</h3>
        ...
        <div className="pib-card-section overflow-hidden">
          <div data-impeccable-disable="content-invisible-at-rest" className="hidden md:grid grid-cols-12 gap-2 px-5 py-3 border-b border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)]">
            <p className="col-span-4 sc-tiny !text-[10px]">Keyword</p>
            <p className="col-span-1 sc-tiny !text-[10px] text-right">Position</p>
            <p className="col-span-1 sc-tiny !text-[10px] text-right">Change</p>
            <p className="col-span-2 sc-tiny !text-[10px] text-right">Volume</p>
            <p className="col-span-2 sc-tiny !text-[10px]">Status</p>
            <p className="col-span-2 sc-tiny !text-[10px] text-right">Actions</p>
          </div>
                  ...
                  <div key={kw.id}>
                    ...
                    >
                      ...
                      <div className="md:col-span-1 text-right tabular-nums text-sm">
                        <p className="md:hidden sc-tiny !text-[9px] mb-1">Position</p>
                      ...
                      <div className="md:col-span-1 text-right">
                        <p className="md:hidden sc-tiny !text-[9px] mb-1">Change</p>
                        <PositionChangePill delta={delta} />
                      </div>

                      <div className="md:col-span-2 text-right tabular-nums text-sm">
                        <p className="md:hidden sc-tiny !text-[9px] mb-1">Volume</p>
                        {kw.volume != null ? kw.volume.toLocaleString('en-ZA') : ' - '}
                      </div>

                      <div className="md:col-span-2">
                        <p className="md:hidden sc-tiny !text-[9px] mb-1">Status</p>
                      ...
                      <div className="md:col-span-2 flex justify-end items-center gap-2" onClick={(e) => e.stopPropagation()}>
                        <button name="keywordtrackerclient-action-65"
                    ...
                    {isExpanded && (
                      <div className="px-5 pb-5 pt-2 bg-[var(--color-pib-surface-2)] border-t border-[var(--color-pib-line)]">
                        <div className="flex items-center justify-between mb-3">
                          <p className="text-xs text-[var(--color-pib-text-muted)]">Position history  -  {kw.keyword}</p>
                          <a
      ...
      {toast && (
        <div
          className={`fixed bottom-5 right-5 z-50 border px-4 py-3 text-sm ${
 toast.ok
