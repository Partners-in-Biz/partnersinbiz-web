...
export default function TerminalPolicyPage() {
  ...
  useEffect(() => { void (async () => {
    try {
      const response = await fetch('/api/v1/portal/settings/terminal-policy'); const body = await response.json()
  ...
  async function save() {
    ...
    try {
      ...
      const response = await fetch('/api/v1/portal/settings/terminal-policy', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ allowedShellArgv }) })
      const body = await response.json(); if (!response.ok) throw new Error(body.error ?? 'Unable to save terminal policy')
  ...
  return <div className="mx-auto max-w-3xl space-y-6" data-module-accent="cyan">
    ...
    <section className="pib-card space-y-4 p-5"><p className="text-sm text-[var(--color-pib-text-muted)]">One exact command per line. Shells, pipes, redirects, quotes, and wildcard syntax are rejected.</p>
      <textarea aria-label="Allowed commands" value={commands} onChange={(event) => setCommands(event.target.value)} className="pib-input min-h-80 w-full font-mono text-sm" spellCheck={false} />
      <div className="flex justify-between gap-3"><button type="button" className="btn-pib-ghost btn-pib-sm" onClick={() => setCommands(defaults.join('\n'))}>Restore safe defaults</button><button type="button" disabled={saving} className="btn-pib-primary btn-pib-sm disabled:opacity-50" onClick={() => void save()}>{saving ? 'Saving…' : 'Save policy'}</button></div>
