/**
 * GET /api/v1/chat-feed/[convId] — list conversation messages through a
 * browser-extension-friendly alias.
 *
 * Some client-side blockers treat `/messages` API paths as blocked network
 * requests. Keep the canonical conversation messages API, but expose this
 * read-only alias so the dynamic chat window can still render the thread.
 */
