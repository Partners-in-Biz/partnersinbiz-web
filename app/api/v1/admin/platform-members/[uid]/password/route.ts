/**
 * PATCH /api/v1/admin/platform-members/[uid]/password
 *
 * Allows a super admin to set a client member's Firebase Auth password.
 */
...
export const PATCH = withAuth('admin', async (req: NextRequest, user, ctx) => {
  if (!isSuperAdmin(user)) return apiError('Only super admins can set platform member passwords', 403)
  ...
  if (doc.data()?.role !== 'client') return apiError('Not a client platform member', 404)
