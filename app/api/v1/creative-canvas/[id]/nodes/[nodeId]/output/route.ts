...
import { attachCreativeCanvasNodeOutput } from '@/lib/creative-canvas/collaboration'
...
export const PUT = withAuth('client', async (req: NextRequest, user: ApiUser, context?: unknown) => {
  ...
  const canvas = await attachCreativeCanvasNodeOutput(id, orgId, nodeId, body, actorFromUser(user))
