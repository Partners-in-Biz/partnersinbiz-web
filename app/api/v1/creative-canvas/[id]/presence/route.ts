...
export const GET = withAuth('client', async (req: NextRequest, user: ApiUser, context?: unknown) => {
  ...
  const presence = await listCreativeCanvasPresence(id, orgId)
...
export const POST = withAuth('client', async (req: NextRequest, user: ApiUser, context?: unknown) => {
  ...
  const presence = await heartbeatCreativeCanvasPresence(id, orgId, body, actorFromUser(user))
  return apiSuccess({ presence: [presence] })
})
