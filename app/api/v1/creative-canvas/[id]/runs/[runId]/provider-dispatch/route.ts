...
import { dispatchCreativeCanvasProviderRun } from '@/lib/creative-canvas/runs'
...
export const PUT = withAuth('client', async (req: NextRequest, user: ApiUser, context?: unknown) => {
  ...
  const run = await dispatchCreativeCanvasProviderRun(runId, orgId, body, actorFromUser(user))
