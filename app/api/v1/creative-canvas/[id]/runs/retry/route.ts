...
import { retryCreativeCanvasProviderRunsForCanvas } from '@/lib/creative-canvas/runs'
...
export const PUT = withAuth('client', async (req: NextRequest, user: ApiUser, context?: unknown) => {
  ...
  try {
    const result = await retryCreativeCanvasProviderRunsForCanvas(id, orgId, actorFromUser(user))
    return apiSuccess(result)
  } catch (error) {
    return apiError(error instanceof Error ? error.message : 'Creative canvas provider batch retry failed', 400)
  }
})
