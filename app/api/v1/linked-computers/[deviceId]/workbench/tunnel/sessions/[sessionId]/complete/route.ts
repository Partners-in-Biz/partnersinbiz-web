...
import { completeTunnelSession } from '@/lib/messages/workbench/tunnel-session-store'
...
function completionError(error: unknown): Response {
  ...
  const publicMessage = status === 404 ? 'Workbench tunnel not found'
    : status === 403 ? 'Linked computer access denied'
      : status === 409 ? 'Workbench tunnel completion is no longer current'
        : 'Linked computer workbench tunnel completion invalid'
  return NextResponse.json({ success: false, error: publicMessage }, { status, headers: noStoreHeaders })
}

/** Device worker reports the tunnel process's terminal outcome: `exited` (own exit code), `killed`, or `failed` (spawn/provider error). */
export async function handleWorkbenchTunnelComplete(
  request: NextRequest,
  deviceId: string,
  sessionId: string,
  authenticate: (request: NextRequest, deviceId: string, rawBody: string) => Promise<DeviceIdentity> = authenticateSignedDeviceRequest,
  complete: typeof completeTunnelSession = completeTunnelSession,
): Promise<Response> {
  try {
    ...
    if (!Number.isSafeInteger(attempt) || attempt < 1
      ...
      throw new Error('workbench: invalid tunnel completion')
    }
...
export const POST = async (request: NextRequest, context: Context) => {
  ...
  return handleWorkbenchTunnelComplete(request, deviceId, sessionId)
}
