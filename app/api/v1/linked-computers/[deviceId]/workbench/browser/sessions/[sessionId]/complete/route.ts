...
import { completeWorkbenchBrowserSession } from '@/lib/messages/workbench/browser-session-store'
...
function completionError(error: unknown): Response {
  ...
  const publicMessage = status === 404 ? 'Workbench browser session not found'
    : status === 403 ? 'Linked computer access denied'
      : status === 409 ? 'Workbench browser session completion is no longer current'
        : 'Linked computer workbench browser session completion invalid'
  return NextResponse.json({ success: false, error: publicMessage }, { status, headers: noStoreHeaders })
}

/** Device worker reports headless Chrome's terminal outcome: `exited` (own exit), `killed`, or `failed` (spawn/CDP error). */
export async function handleWorkbenchBrowserSessionComplete(
  request: NextRequest,
  deviceId: string,
  sessionId: string,
  authenticate: (request: NextRequest, deviceId: string, rawBody: string) => Promise<DeviceIdentity> = authenticateSignedDeviceRequest,
  complete: typeof completeWorkbenchBrowserSession = completeWorkbenchBrowserSession,
): Promise<Response> {
  try {
    ...
    if (!Number.isSafeInteger(attempt) || attempt < 1
      ...
      throw new Error('workbench: invalid browser session completion')
    }
...
export const POST = async (request: NextRequest, context: Context) => {
  ...
  return handleWorkbenchBrowserSessionComplete(request, deviceId, sessionId)
}
