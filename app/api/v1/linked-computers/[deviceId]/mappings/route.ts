...
export async function handleDeviceMapping(req: NextRequest, user: { uid: string }, deviceId: string, put = putWorkspaceMapping): Promise<Response> {
  try {
    ...
    if (![body.orgId, body.workspaceId, body.label].every((v) => typeof v === 'string') || !['active', 'stale', 'missing', 'paused', 'removed'].includes(body.status)) throw new Error('linked computers: invalid mapping')
    await put({ mappingId: randomUUID(), deviceId, orgId: body.orgId, workspaceId: body.workspaceId, actorUserId: user.uid, label: body.label, status: body.status })
