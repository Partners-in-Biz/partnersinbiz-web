...
import { revertProjectionDrift } from '@/lib/agent-rooms/projection'
...
export const POST = withAuth('client', async (_req: NextRequest, user, ctx) => {
  ...
  try {
    const result = await revertProjectionDrift({
      orgId,
      projectionId,
      actorUserId: user.uid,
    })
    return apiSuccess({ projection: result.projection, jobId: result.jobId })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'agent rooms: revert failed'
