...
/**
 * GET /api/v1/quotes/[id]/html
 *
 * Returns print-friendly quote HTML (reuses invoice HTML layout) for Messages Context Dock.
 */
export const GET = withCrmAuth<RouteCtx>('viewer', async (_req, ctx, routeCtx) => {
  ...
  try {
    const html = generateInvoiceHtml(invoiceLikeFromQuoteRecord(data as unknown as Record<string, unknown>, id))
    ...
    return apiError('Failed to generate quote HTML', 500)
  }
})
