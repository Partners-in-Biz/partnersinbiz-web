...
function buildDecisionDataOperatingRuleContext(): string {
  return [
    ...
    'For Partners in Biz parent-workspace growth, CRM pipeline, Marketing Studio, failed-social recovery, agent review, or approval-queue questions, use the read-only gatherer first: GET /api/v1/agent/growth-command-queue with the current orgId or X-Org-Id. Treat its sourceReports and queue as the stored-data input for the chat answer.',
    ...
    '5. Return the decision, evidence, reusable workflow, and next actions in this dynamic chat window.',
    ...
    'Temporary HTML is allowed only as a throw-away linked/attached artifact inside the chat thread; never make it the only place where the answer lives.',
