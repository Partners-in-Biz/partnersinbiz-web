...
export default function PortalEmailPage() {
  ...
  async function loadMessages() {
    ...
    const res = await fetch(`/api/v1/admin/mailbox/messages?${params.toString()}`)
    const body = await res.json()
  ...
  async function saveAccount(event: FormEvent) {
    ...
    if (accountForm.provider === 'google') {
      ...
      window.location.href = `/api/v1/admin/mailbox/google/authorize?${params.toString()}`
      return
    }
    const payload = {
      provider: 'smtp_imap',
  ...
  async function submitCompose(action: 'send' | 'draft') {
    setError(null)
    const res = await fetch('/api/v1/admin/mailbox/messages', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...compose,
  ...
  async function updateMessage(id: string, patch: Record<string, unknown>) {
    await fetch(`/api/v1/admin/mailbox/messages/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
        ...
        <form onSubmit={saveAccount} className="bento-card !p-5 space-y-4">
          ...
          {accountForm.provider === 'smtp_imap' ? (
            ...
            <div className="rounded-lg border border-[var(--color-pib-line)] bg-white/[0.03] p-4 text-sm text-[var(--color-pib-text-muted)]">
              Continue with Google to approve mailbox access. PiB stores the OAuth token on your admin profile after Google returns you here.
            </div>
          )}
