...
const RUN_STATUS_PILL: Record<string, string> = {
  ...
  awaiting_approval: 'bg-[var(--st-warning)]/10 text-[var(--st-warning)]',
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="grid gap-3 md:grid-cols-3">
        <div className="pib-stat-card p-4">
          ...
          <p className="mt-1.5 text-2xl tabular-nums text-[var(--color-pib-text)]">{activeLoops.length}</p>
        ...
        <div className="pib-stat-card p-4">
          ...
          <p className="mt-1.5 text-2xl tabular-nums text-[var(--color-pib-text)]">{guardedLoops.length}</p>
        ...
        <div className="pib-stat-card p-4">
          ...
          <p className="mt-1.5 text-2xl tabular-nums text-[var(--color-pib-text)]">{plannedLoops.length}</p>
          <p className="mt-1.5 text-sm text-[var(--color-pib-text-muted)]">SEO / CRM and lead-response loops queued after visibility/governance hardening.</p>
        </div>
      </section>
      ...
      <section className="pib-card p-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-medium text-[var(--color-pib-text)]">Recent loop runs</h2>
          ...
          <span className="rounded-md bg-white/5 px-3 py-1 text-sm font-medium text-[var(--color-pib-text-muted)]">{liveRuns.length} shown</span>
        </div>
        ...
        {liveRunsError ? (
          ...
          <div className="mt-5 rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-4 text-sm text-[var(--color-pib-text-muted)]">
          ...
          <div className="mt-5 space-y-3">
            {liveRuns.map((run) => (
              <div key={run.id} className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-medium text-[var(--color-pib-text)]">{run.loopName}</p>
                    <p className="mt-1 text-xs text-[var(--color-pib-text-muted)]">
                      {run.triggerSource} · {run.ownerAgentId} → {run.reviewerAgentId} · {formatWhen(run.updatedAt)}{run.dryRun ? ' · dry-run' : ''}
                    </p>
                  </div>
                  <span className={`rounded-md px-3 py-1 text-xs font-medium ${statusPill(run.status)}`}>
                ...
                {run.needsHumanJudgment ? (
                  <span className="mt-3 inline-flex rounded-md bg-[var(--st-warning)]/10 px-2.5 py-1 text-xs font-medium text-[var(--st-warning)]">Needs human judgment</span>
                ) : null}
              </div>
            ))}
          </div>
      ...
      <section className="pib-card p-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-medium text-[var(--color-pib-text)]">Full-loop execution layer</h2>
            <p className="mt-2 max-w-3xl text-sm text-[var(--color-pib-text-muted)]">
          ...
          <span className={`rounded-md px-3 py-1 text-sm font-medium ${statusPill(DEMO_RUN.status)}`}>{DEMO_RUN.status.replace(/_/g, ' ')}</span>
        </div>
        <div className="mt-5 grid gap-4 lg:grid-cols-3">
          <div className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-4">
            <p className="text-sm font-medium text-[var(--color-pib-text)]">Run decision</p>
            <p className="mt-2 text-sm text-[var(--color-pib-text-muted)]">{DEMO_RUN.decision}</p>
          </div>
          <div className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-4">
            <p className="text-sm font-medium text-[var(--color-pib-text)]">Progress signal</p>
          ...
          <div className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-4">
            <p className="text-sm font-medium text-[var(--color-pib-text)]">Needs human judgment</p>
          ...
          <div className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-4">
            <p className="text-sm font-medium text-[var(--color-pib-text)]">Candidate summary</p>
            <p className="mt-2 text-sm text-[var(--color-pib-text-muted)]">{DEMO_RUN.candidateSummary}</p>
          </div>
          <div className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-4">
            <p className="text-sm font-medium text-[var(--color-pib-text)]">Last meaningful action</p>
            <p className="mt-2 text-sm text-[var(--color-pib-text-muted)]">{DEMO_RUN.observability.lastMeaningfulAction}</p>
          </div>
          <div className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-4">
            <p className="text-sm font-medium text-[var(--color-pib-text)]">Approval gates</p>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          {DEMO_RUN.proposedActions.map((action) => (
            <div key={action.id} className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface)] p-3">
              <p className="text-sm font-medium text-[var(--color-pib-text)]">{action.label}</p>
      ...
      <section className="pib-card p-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-medium text-[var(--color-pib-text)]">Loop templates from the community pattern library</h2>
            <p className="mt-2 max-w-3xl text-sm text-[var(--color-pib-text-muted)]">
          ...
          <div className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-4 text-sm text-[var(--color-pib-text-muted)] md:min-w-72">
            <p className="font-medium text-[var(--color-pib-text)]">Source monitor</p>
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-2">
          {LOOP_TEMPLATE_LIBRARY.map((template) => (
            <article key={template.id} className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="pib-label">{template.category} · {template.recommendedOwnerAgentId} → {template.recommendedReviewerAgentId}</p>
                  <h3 className="mt-2 text-lg font-medium text-[var(--color-pib-text)]">{template.name}</h3>
                </div>
                <Link href={`${LOOP_TEMPLATE_SOURCE.siteUrl}loops/${template.sourceSlug}/`} className="rounded-md bg-white/5 px-3 py-1 text-xs font-medium text-[var(--color-pib-text-muted)]">source</Link>
              </div>
              ...
              <dl className="mt-4 grid gap-3 text-sm md:grid-cols-2">
                <div>
                  <dt className="font-medium text-[var(--color-pib-text)]">Trigger</dt>
                  <dd className="mt-1 text-[var(--color-pib-text-muted)]">{template.trigger}</dd>
                </div>
                <div>
                  <dt className="font-medium text-[var(--color-pib-text)]">Action</dt>
                  <dd className="mt-1 text-[var(--color-pib-text-muted)]">{template.action}</dd>
                </div>
                <div>
                  <dt className="font-medium text-[var(--color-pib-text)]">Proof</dt>
                  <dd className="mt-1 text-[var(--color-pib-text-muted)]">{template.proof}</dd>
                </div>
                <div>
                  <dt className="font-medium text-[var(--color-pib-text)]">Stop condition</dt>
              ...
              <div className="mt-4 rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface)] p-3">
                <p className="text-sm font-medium text-[var(--color-pib-text)]">Starter prompt</p>
                <p className="mt-2 text-sm text-[var(--color-pib-text-muted)]">{template.starterPrompt}</p>
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
                {template.guardrails.map((guardrail) => (
                  <span key={guardrail} className="rounded-md bg-[var(--st-warning)]/10 px-2.5 py-1 text-xs font-medium text-[var(--st-warning)]">{guardrail}</span>
                ))}
              </div>
            </article>
          ))}
      ...
      <section className="pib-card p-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-medium text-[var(--color-pib-text)]">Task eligibility explainer</h2>
            <p className="mt-2 max-w-3xl text-sm text-[var(--color-pib-text-muted)]">
          ...
          <span className={`rounded-md px-3 py-1 text-sm font-medium ${readiness.eligible ? 'bg-emerald-500/10 text-emerald-300' : 'bg-[var(--st-warning)]/10 text-[var(--st-warning)]'}`}>
            {readiness.eligible ? 'Eligible' : 'Blocked'}
          </span>
        </div>
        <div className="mt-5 rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-4">
          <p className="font-medium text-[var(--color-pib-text)]">{readiness.summary}</p>
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {readiness.reasons.map((reason) => (
              <div key={reason.code} className="rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface)] p-3">
                <p className="text-sm font-medium text-[var(--color-pib-text)]">{reason.label}</p>
                <p className="mt-1 text-xs uppercase tracking-wide text-[var(--color-pib-text-muted)]">{reason.code}</p>
              </div>
            ))}
          </div>
          <div className="mt-4">
            <p className="text-sm font-medium text-[var(--color-pib-text)]">Required evidence for this risk level</p>
          ...
          <article key={loop.id} className="pib-card p-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h2 className="mt-2 text-xl font-medium text-[var(--color-pib-text)]">{loop.name}</h2>
              </div>
            ...
            <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
              ...
              <div>
                <dt className="font-medium text-[var(--color-pib-text)]">No-progress policy</dt>
                <dd className="mt-1 text-[var(--color-pib-text-muted)]">{loop.loopContract.noProgressPolicy}</dd>
              </div>
              <div>
                <dt className="font-medium text-[var(--color-pib-text)]">Budget guardrail</dt>
                <dd className="mt-1 text-[var(--color-pib-text-muted)]">{loop.loopContract.budgetGuardrail}</dd>
              </div>
              <div>
                <dt className="font-medium text-[var(--color-pib-text)]">Owner / reviewer</dt>
                <dd className="mt-1 text-[var(--color-pib-text-muted)]">{loop.ownerAgentId} → {loop.reviewerAgentId}</dd>
              </div>
              <div>
                <dt className="font-medium text-[var(--color-pib-text)]">Allowed actions</dt>
                <dd className="mt-1 text-[var(--color-pib-text-muted)]">{loop.allowedActions.join(', ')}</dd>
              </div>
              <div>
                <dt className="font-medium text-[var(--color-pib-text)]">Stale threshold</dt>
                <dd className="mt-1 text-[var(--color-pib-text-muted)]">{loop.staleThreshold}</dd>
              </div>
            </dl>
            <div className="mt-4 rounded-md border border-[var(--color-pib-line)] bg-[var(--color-pib-surface-2)] p-3">
              <p className="text-sm font-medium text-[var(--color-pib-text)]">Last decision</p>
            ...
            <div className="mt-3 rounded-md border border-emerald-500/20 bg-emerald-500/5 p-3">
              <p className="text-sm font-medium text-[var(--color-pib-text)]">Operator-facing value</p>
            ...
            {loop.approvalGates.length > 0 ? (
              <div className="mt-4 flex flex-wrap gap-2">
                {loop.approvalGates.map((gate) => (
                  <span key={gate} className="rounded-md bg-[var(--st-warning)]/10 px-2.5 py-1 text-xs font-medium text-[var(--st-warning)]">{gate}</span>
                ))}
              </div>
            ) : null}
          </article>
