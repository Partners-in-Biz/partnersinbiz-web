...
describe('UnifiedChat mobile conversation first paint', () => {
  ...
  it('hides sandwich chrome on a phone and still reaches it from overflow', async () => {
    installViewport(390)
    installFetch()
    renderMessages('bot')
    ...
    expect(screen.queryByTestId('command-session-badge')).not.toBeInTheDocument()
    expect(screen.queryByTestId('conversation-mobile-subtitle')).not.toBeInTheDocument()
    expect(screen.queryByTestId('connection-where-chip-mobile')).not.toBeInTheDocument()
    expect(screen.queryByTestId('bot-computer-strip')).not.toBeInTheDocument()
    expect(screen.queryByTestId('conversation-context-strip')).not.toBeInTheDocument()
    expect(screen.queryByTestId('context-pulse')).not.toBeInTheDocument()
    expect(screen.queryByTestId('agent-workbench-rail')).not.toBeInTheDocument()
    expect(screen.queryByTestId('hermes-runtime-control-bar')).not.toBeInTheDocument()
    expect(screen.queryByTestId('chat-context-toolbar')).not.toBeInTheDocument()
    expect(screen.queryByLabelText('Approval mode')).not.toBeInTheDocument()
    expect(screen.queryByLabelText('Runtime thinking effort')).not.toBeInTheDocument()
    ...
    expect(screen.getByTestId('overflow-command-session-badge')).toHaveTextContent('Command session')
