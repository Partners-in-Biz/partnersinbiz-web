...
describe('LifeOsPlanningWorkbench', () => {
  it('renders vision to quarterly, weekly, daily, and progress layers', () => {
    ...
    expect(screen.getByText(/Share only what feels useful/i)).toBeInTheDocument()
    expect(screen.getByText(/You do not need to include medical, mental-health, crisis, legal, financial, or other highly sensitive details/i)).toBeInTheDocument()
    expect(screen.getByText(/We store Life OS entries only after consent/i)).toBeInTheDocument()
    expect(screen.getByText('AI coach workflow')).toBeInTheDocument()
    expect(screen.getByText('Prompt guardrails')).toBeInTheDocument()
    expect(screen.getByText('Plan suggestions')).toBeInTheDocument()
    expect(screen.getByText('Obstacle diagnosis')).toBeInTheDocument()
    expect(screen.getByText('Reflection summary')).toBeInTheDocument()
    expect(screen.getByText('Experiment recommendations')).toBeInTheDocument()
    expect(screen.getByText('Safety boundaries')).toBeInTheDocument()
    expect(screen.getByText(/not a medical, mental-health, legal, or crisis professional/i)).toBeInTheDocument()
    expect(screen.getByText(/not therapy, medical care, crisis support, legal advice, or financial advice/i)).toBeInTheDocument()
    expect(screen.getByText(/If you might be in immediate danger, contact local emergency services or a trusted person now/i)).toBeInTheDocument()
    expect(screen.getByText('Insights dashboard signals')).toBeInTheDocument()
    expect(screen.getByText('Experiment loop')).toBeInTheDocument()
    expect(screen.getByText(/Hypothesis:/)).toBeInTheDocument()
    expect(screen.getByText(/Duration: 2026-06-15 → 2026-06-21/)).toBeInTheDocument()
    expect(screen.getByText(/Evidence: 4 morning walks completed/)).toBeInTheDocument()
    expect(screen.getByText(/Decision: Iterate/)).toBeInTheDocument()
    expect(screen.getByText(/Adaptation suggestion:/)).toBeInTheDocument()
