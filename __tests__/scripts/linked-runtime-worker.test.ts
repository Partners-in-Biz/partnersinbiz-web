...
it('records heartbeat liveness before every signed heartbeat attempt',async()=>{let stopped=false;const attempts:number[]=[];await heartbeatForever(async()=>{stopped=true},()=>stopped,60_000,async()=>undefined,async()=>{attempts.push(1)});expect(attempts).toEqual([1])})
