...
describe('linked runtime executable core', () => {
  ...
  it('rejects malformed, wrong-channel and downgrade release manifests',()=>{const crypto=require('node:crypto'),mk=(m:any)=>sign(null,Buffer.from(canonicalJson(m)),keys.privateKey).toString('base64url'),manifest={...base,sha256:crypto.createHash('sha256').update(payload).digest('hex')};expect(()=>verifyRelease({...manifest,version:'1.x'},mk({...manifest,version:'1.x'}),payload,keys.publicKey,{platform:'macos',architecture:'arm64',currentVersion:'2.0.0',channel:'stable'})).toThrow(/semver/);expect(()=>verifyRelease({...manifest,channel:'beta'},mk({...manifest,channel:'beta'}),payload,keys.publicKey,{platform:'macos',architecture:'arm64',currentVersion:'2.0.0',channel:'stable'})).toThrow(/channel/);expect(()=>verifyRelease({...manifest,version:'1.9.0',minimumVersion:'1.0.0'},mk({...manifest,version:'1.9.0',minimumVersion:'1.0.0'}),payload,keys.publicKey,{platform:'macos',architecture:'arm64',currentVersion:'2.0.0',channel:'stable'})).toThrow(/downgrade/)})
  it('verifies the checked-in build/package fixture',()=>{const f=JSON.parse(fs.readFileSync('runtime-installers/fixtures/release-valid.json','utf8')),publicKey=fs.readFileSync('runtime-installers/fixtures/release-public-key.txt');const {signature,payload,...manifest}=f;expect(()=>verifyRelease(manifest,signature,Buffer.from(payload,'base64'),publicKey,{platform:'windows',architecture:'x64',currentVersion:'1.0.0'})).not.toThrow()})
  ...
  it('implements SemVer 2 prerelease precedence and rejects leading zeroes',()=>{const crypto=require('node:crypto'),mk=(m:any)=>sign(null,Buffer.from(canonicalJson(m)),keys.privateKey).toString('base64url'),manifest={...base,minimumVersion:'1.0.0',sha256:crypto.createHash('sha256').update(payload).digest('hex')};const host={platform:'macos',architecture:'arm64',currentVersion:'2.0.0',channel:'stable'};for(const version of ['01.0.0','1.0.0-01','1.0'])expect(()=>verifyRelease({...manifest,version},mk({...manifest,version}),payload,keys.publicKey,host)).toThrow(/semver/);expect(()=>verifyRelease({...manifest,version:'2.0.0-beta.2'},mk({...manifest,version:'2.0.0-beta.2'}),payload,keys.publicKey,host)).toThrow(/downgrade/);expect(()=>verifyRelease({...manifest,version:'2.0.1-rc.1'},mk({...manifest,version:'2.0.1-rc.1'}),payload,keys.publicKey,host)).not.toThrow()})
})
