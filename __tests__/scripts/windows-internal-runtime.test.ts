...
describe('internal Windows staff runtime channel', () => {
  ...
  it('keeps internal releases isolated from the future public channel', () => {
    ...
    expect(workflow).toMatch(/certutil\.exe -user -f -addstore \$StoreName/)
    expect(workflow).toMatch(/Cert:\\CurrentUser\\\$StoreName/)
    ...
    expect(workflow).toMatch(/name: Install internal trust non-interactively\n\s+timeout-minutes: 2/)
