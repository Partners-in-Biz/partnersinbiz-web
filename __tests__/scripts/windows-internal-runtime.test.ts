...
describe('internal Windows staff runtime channel', () => {
  ...
  it('requires explicit staff intent before modifying machine trust', () => {
    ...
    expect(bootstrap).toMatch(/certutil\.exe -silent -f -addstore \$StoreName \$CertificatePath/)
  ...
  it('keeps internal releases isolated from the future public channel', () => {
    ...
    expect(workflow).toMatch(/\$Signature\.Status -notin @\('Valid','NotTrusted'\)/)
    expect(workflow).not.toMatch(/certutil\.exe/)
    ...
    expect(workflow).not.toMatch(/name: Install internal trust/)
