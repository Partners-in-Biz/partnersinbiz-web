...
describe('POST /api/v1/crm/contacts', () => {
  ...
  it('persists website, GitHub, and extra links on create', async () => {
    const member = seedOrgMember('org-test', 'uid-member', { role: 'member' })
    const capturedDocSet = jest.fn().mockResolvedValue(undefined)
    stageAuth(member, {}, { capturedDocSet })
    ...
    const res = await POST(req)
    expect(res.status).toBe(201)
    expect(capturedDocSet).toHaveBeenCalledWith(expect.objectContaining({
      website: 'ava.example',
      githubUrl: 'https://github.com/ava',
      youtubeUrl: '',
  ...
  it('ignores linkedUserId on create so partner links stay server-owned', async () => {
    ...
    expect(res.status).toBe(201)
    expect(capturedDocSet).toHaveBeenCalledWith(expect.not.objectContaining({
