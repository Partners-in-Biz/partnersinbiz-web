...
describe('GET /api/v1/invoices', () => {
  ...
  it('platform admin with activeOrgId is scoped to that workspace (sent invoices)', async () => {
    // Simulates Stean (platform admin) in Humanaut workspace
    mockUser = {
      ...
      // No allowedOrgIds = unrestricted platform admin
    }
    ...
    mockInvoiceGet.mockResolvedValue({
      docs: [
        // Humanaut-issued invoice (should appear)
        { id: 'humanaut-inv', data: () => ({ orgId: 'humanaut-org', sourceOrgId: 'humanaut-org', invoiceNumber: 'HUM-001', createdAt: { seconds: 30 } }) },
        // PiB invoice to different client (should NOT appear)
        { id: 'pib-to-other', data: () => ({ orgId: 'pib-platform-owner', sourceOrgId: 'pib-platform-owner', recipientOrgId: 'other-client', invoiceNumber: 'SAA-002', createdAt: { seconds: 20 } }) },
      ],
    })
    ...
    expect(res.status).toBe(200)
    const body = await res.json()
    // MUST only see Humanaut invoices
    expect(body.data.map((inv: { id: string }) => inv.id)).toEqual(['humanaut-inv'])
    // MUST NOT see PiB invoices to other clients
    expect(body.data.map((inv: { id: string }) => inv.id)).not.toContain('pib-to-other')
    // Verify query was scoped to humanaut-org
    expect(mockInvoiceWhere).toHaveBeenCalledWith('orgId', '==', 'humanaut-org')
  })

  it('platform admin with activeOrgId is scoped to that workspace (received invoices)', async () => {
    mockUser = {
      uid: 'platform-admin-1',
      role: 'admin',
      orgId: 'pib-platform-owner',
    ...
    mockInvoiceGet
      .mockResolvedValueOnce({
        docs: [
          { id: 'pib-to-humanaut', data: () => ({ recipientOrgId: 'humanaut-org', sourceOrgId: 'pib-platform-owner', invoiceNumber: 'PAR-001', createdAt: { seconds: 30 } }) },
        ],
      })
    ...
    // MUST be scoped to humanaut-org
    expect(mockInvoiceWhere).toHaveBeenCalledWith('recipientOrgId', '==', 'humanaut-org')
  })

  it('platform admin WITHOUT activeOrgId can query globally (backward compat)', async () => {
    // Admin using API directly, not through portal
    mockUser = {
      uid: 'platform-admin-1',
      role: 'admin',
      orgId: 'pib-platform-owner',
    ...
    mockInvoiceGet.mockResolvedValue({
      docs: [
        { id: 'inv-1', data: () => ({ orgId: 'client-a', invoiceNumber: 'INV-001', createdAt: { seconds: 30 } }) },
        { id: 'inv-2', data: () => ({ orgId: 'client-b', invoiceNumber: 'INV-002', createdAt: { seconds: 20 } }) },
      ],
    })
    ...
    expect(res.status).toBe(200)
    const body = await res.json()
    // Global admin can see all (no activeOrgId = no portal workspace scoping)
