...
describe('AR/AP depth service golden path', () => {
  test('invoice → partial pay → credit note → statement totals', async () => {
    ...
    expect(store.invoices.get(invoice.id)?.outstandingMinor).toBe(6_500)

    const credit = await service.createCustomerCreditNote(actor, {
    ...
    await service.issueCustomerCreditNote(actor, {
      ...scope, creditNoteId: credit.id, expectedVersion: credit.version, ...request('cn-issue'),
    })
    await service.applyCustomerCreditNote(actor, {
      ...scope, id: 'cna-1', creditNoteId: credit.id, invoiceId: invoice.id, appliedMinor: 1_725, expectedVersion: 0, ...request('cn-apply'),
    })
    expect(store.creditNotes.get(credit.id)?.status).toBe('applied')
    expect(store.invoices.get(invoice.id)?.outstandingMinor).toBe(4_775)

    const statement = await service.createCounterpartyStatement(actor, {
    ...
    const aging = service.buildAgingReport(actor, { ...scope, role: 'customer', asOfDate: '2026-08-15', currency: 'ZAR' })
    expect(aging.totalOutstandingMinor).toBe(4_775)

    const schedule = await service.createRecurringSchedule(actor, {
    ...
    const generated = await service.generateRecurringSchedule(actor, {
    ...
    await service.addDocumentAttachment(actor, {
    ...
    const bulk = await service.bulkIssueDocuments(actor, {
      ...scope, id: 'bulk-1',
      targets: [{ type: 'customer_invoice', id: 'inv-rec-1', expectedVersion: store.invoices.get('inv-rec-1')!.version, controlAccountId: 'ar' }],
      ...request('bulk-issue'),
    })
  ...
  test('supplier debit note applies against bill and bulk allocate works', async () => {
    ...
    const debit = await service.createSupplierDebitNote(actor, {
    ...
    const bulkAlloc = await service.bulkAllocatePayments(actor, {
    ...
    expect(store.payments.get(payment.id)?.unallocatedMinor).toBe(0)
  })
})
