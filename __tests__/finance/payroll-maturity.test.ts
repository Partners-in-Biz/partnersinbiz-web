...
describe('payroll leave pure helpers', () => {
  test('converts days to hours and projects cut-off calendar', () => {
    ...
    const view = projectPayCalendar({
      calendars: [{
        id: 'cal-1', orgId, legalEntityId: scope.legalEntityId, bookId: scope.bookId,
        schemaVersion: 1, version: 1, createdAt: '', createdBy: '', updatedAt: '', updatedBy: '',
        code: 'M', name: 'Monthly', frequency: 'monthly', status: 'active',
      }],
      periods: [{
        id: 'p1', orgId, legalEntityId: scope.legalEntityId, bookId: scope.bookId,
        schemaVersion: 1, version: 1, createdAt: '', createdBy: '', updatedAt: '', updatedBy: '',
        calendarId: 'cal-1', frequency: 'monthly', label: 'Aug', periodStart: '2026-08-01', periodEnd: '2026-08-31',
        cutOffAt: '2026-08-20T12:00:00.000Z', payDate: '2026-08-25', taxYearLabel: '2025/26', status: 'open',
      }],
  ...
  test('approved unpaid leave merges into calc when explicit leave omitted', () => {
    const fromApproved = approvedLeaveToCalcInputs([
      {
        id: 'lv1', status: 'approved', payEffect: 'unpaid',
        startDate: '2026-08-10', endDate: '2026-08-11', hours: 16,
      },
  ...
  test('payslip pack is download-only with no egress flags', () => {
    const payslip = {
      ...
      rendered: {
        ...
        currency: 'ZAR',
        lines: [{
          lineId: 'l1', componentCode: 'BASE', kind: 'base_salary', description: 'Salary',
          quantity: 1, unitAmountMinor: 100000, amountMinor: 100000,
          taxTreatment: 'taxable', uifTreatment: 'include', sdlTreatment: 'include',
        }],
        totals: {
          ordinaryMinor: 100000, overtimeMinor: 0, leavePaidMinor: 0, leaveUnpaidReductionMinor: 0,
          bonusMinor: 0, commissionMinor: 0, allowancesMinor: 0, benefitsMinor: 0,
          grossEarningsMinor: 100000, taxableEarningsMinor: 100000, preTaxDeductionsMinor: 0,
          postTaxDeductionsMinor: 0, payeMinor: 10000, uifEmployeeMinor: 1000, uifEmployerMinor: 1000,
          sdlEmployerMinor: 1000, netPayMinor: 89000, employerCostMinor: 102000,
        },
        netPayMinor: 89000,
      },
      immutable: true,
...
describe('payroll maturity service integration', () => {
  test('leave affects calc; ESS sees own payslip only; pack download only', async () => {
    ...
    const term = await calc.createTermVersion(clerk, {
      ...
      rateMinor: 4_000_000, standardHoursPerPeriod: 160,
    ...
    const period = await calc.createPayPeriod(clerk, {
      id: 'per-1', ...scope, calendarId: calendar.id, label: '2026-08',
      periodStart: '2026-08-01', periodEnd: '2026-08-31', payDate: '2026-08-25',
    ...
    const leaveType = await leave.createLeaveType(clerk, {
      id: 'lt-unpaid', ...scope, code: 'UNPAID', name: 'Unpaid leave', unit: 'hours',
      payEffect: 'unpaid', hoursPerDay: 8, accrues: false, expectedVersion: 0, ...request('lt'),
    })
    await leave.setLeaveBalance(clerk, {
      id: 'lb-1', ...scope, employeeId: employee.id, leaveTypeId: leaveType.id,
      balanceQuantity: 40, asOfDate: '2026-08-01', expectedVersion: 0, ...request('lb'),
    })
    const leaveRec = await leave.requestLeave(clerk, {
      id: 'lv-1', ...scope, employeeId: employee.id, leaveTypeId: leaveType.id,
      startDate: '2026-08-10', endDate: '2026-08-10', unit: 'hours', quantity: 8,
      expectedVersion: 0, ...request('lv'),
    })
    ...
    const baseline = await calc.calculateEmployee(clerk, {
      ...
      leave: [],
      expectedVersion: 0, ...request('calc-base'),
    })
    // Force explicit empty leave vs approved merge: pass undefined leave so approved unpaid applies
    // First calculation used leave: [] which merges to approved (empty array falls through)
    expect(baseline.result.totals.leaveUnpaidReductionMinor).toBeGreaterThan(0)

    now = '2026-08-21T10:00:00.000Z'
    const run = await runs.createPayRun(clerk, {
      id: 'run-1', ...scope, calendarId: calendar.id, payPeriodId: period.id,
      ruleVersionId: rule.id, label: 'Aug 2026', expectedVersion: 0, ...request('run'),
    })
    await runs.addItem(clerk, {
      id: 'item-1', ...scope, payRunId: run.id, calculationId: baseline.id,
      expectedVersion: 0, ...request('item'),
    })
    await runs.freezeInputs(clerk, { ...scope, payRunId: run.id, expectedVersion: 1, ...request('freeze') })
    await runs.submitForReview(clerk, { ...scope, payRunId: run.id, expectedVersion: 2, ...request('submit') })
    runs.registerApproval(approval('ap-run', 'payroll.run.approve'))
    const locked = await runs.approveAndLock(approver, {
      ...scope, payRunId: run.id, expectedVersion: 3, approvalId: 'ap-run', reason: 'ok', ...request('lock'),
    })
    ...
    const essPayslip = runs.getPayslip(employeeUser, scope, payslipId)
    expect(essPayslip.id).toBe(payslipId)
    ...
    const pack = await leave.buildPayslipPack(employeeUser, {
      id: 'pack-1', ...scope, payslipId, expectedVersion: 0, ...request('pack'),
    })
    expect(pack.externalEgressAllowed).toBe(false)
    expect(pack.autoSent).toBe(false)
    expect(pack.sarsSubmissionInitiated).toBe(false)
    ...
    const projection = leave.listLeaveBundle(scope).calendarProjection
    expect(projection[0].cutoffStatus).toBe('past_cutoff')
  })
})
