...
describe('portal marketing child scope standard', () => {
  it('keeps brand, capture, sender-domain, comms, and contact workspaces scoped from CRM company routes', () => {
    ...
    const broadcastAnalyticsWorkspace = source('components/email-analytics/BroadcastAnalyticsWorkspace.tsx')
    const sequenceAnalyticsWorkspace = source('components/email-analytics/SequenceAnalyticsWorkspace.tsx')
    ...
    expect(emailAnalytics).toContain('sourceCompanyId?: string')
    expect(emailAnalytics).toContain('sourceCompanyName?: string')
    expect(emailAnalytics).toContain('sourceCompanyId: params?.sourceCompanyId')
    expect(emailAnalytics).toContain('sourceCompanyName: params?.sourceCompanyName')
    expect(emailAnalyticsDashboard).toContain('scopedApiPath')
    expect(emailAnalyticsDashboard).toContain('scopedPortalPath')
    expect(emailAnalyticsDashboard).toContain('sourceCompanyId: orgScope?.sourceCompanyId')
    expect(emailAnalyticsDashboard).toContain('sourceCompanyName: orgScope?.sourceCompanyName')
    expect(broadcastAnalyticsWorkspace).toContain('sourceCompanyId?: string')
    expect(broadcastAnalyticsWorkspace).toContain('sourceCompanyName?: string')
    expect(sequenceAnalyticsWorkspace).toContain('sourceCompanyId?: string')
    expect(sequenceAnalyticsWorkspace).toContain('sourceCompanyName?: string')
