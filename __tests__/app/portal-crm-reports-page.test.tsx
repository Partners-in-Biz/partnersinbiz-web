...
describe('Portal CRM reports page', () => {
  ...
  it('renders activity type keys as readable report labels', async () => {
    ;(global.fetch as jest.Mock).mockImplementation((url: RequestInfo | URL) => {
      const path = String(url)
      if (path === '/api/v1/crm/reports/funnel') {
        return apiResponse({
      ...
      if (path === '/api/v1/crm/reports/activity-summary?days=30') {
        return apiResponse({
          byType: { email_sent: 5, meeting_scheduled: 3 },
          total: 8,
          perDay: [{ date: '2026-05-29', count: 8 }],
          since: '2026-04-29',
          days: 30,
        })
      }
    ...
    expect(screen.queryByText('Meeting_scheduled')).not.toBeInTheDocument()
  })
  ...
  it('turns an empty contact stage mix into a stage classification action', async () => {
    ;(global.fetch as jest.Mock).mockImplementation((url: RequestInfo | URL) => {
      const path = String(url)
      if (path === '/api/v1/crm/reports/funnel') {
        return apiResponse({
    ...
    expect(await screen.findByText('Stage mix missing')).toBeInTheDocument()
    expect(screen.getByText('Classify contacts into funnel stages')).toBeInTheDocument()
    expect(
      screen.getByText(
    ...
    const stageLink = screen.getByRole('link', { name: 'Open contacts to classify missing stage mix' })
    expect(stageLink).toHaveAttribute('href', '/portal/contacts?create=contact')
  })
