...
describe('PortalData', () => {
  ...
  it('names data export commands without decorative icon text', () => {
    ...
    expect(screen.queryByRole('link', { name: 'download Download CSV' })).not.toBeInTheDocument()
    expect(screen.queryByRole('link', { name: 'code Download JSON' })).not.toBeInTheDocument()
  })
  ...
  it('preserves the active company workspace when starting an export', () => {
    ...
    const csvExport = screen.getByRole('link', { name: 'Download CSV' })

    expect(csvExport).toHaveAttribute(
      'href',
      expect.stringMatching(
        /^\/api\/v1\/portal\/data-export\?format=csv&from=\d{4}-\d{2}-\d{2}&to=\d{4}-\d{2}-\d{2}&orgId=lumen-org&companyId=company-1$/,
      ),
    )
  })
