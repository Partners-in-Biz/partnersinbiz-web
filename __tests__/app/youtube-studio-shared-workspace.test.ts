...
describe('youtube studio shared workspace standard', () => {
  it('keeps portal on the YouTube Studio workspace and top-level admin on governance controls', () => {
    ...
    const workQueueLib = source('lib/youtube-studio/work-queue.ts')
    ...
    expect(portalWorkspace).toContain('Request a PiB video')
    expect(portalWorkspace).toContain('ChannelChoices')
    expect(portalWorkspace).toContain('buildRequestHelpText')
    ...
    expect(workQueueLib).toContain('Needs your input')
    expect(workQueue).toContain('WorkQueueGroup')
