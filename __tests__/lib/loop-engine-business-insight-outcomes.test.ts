...
describe('business insight outcome measurement', () => {
  ...
  it('refreshes supported document metrics before measuring due actions that do not have a latest value yet', async () => {
    mockGet.mockResolvedValue({
      docs: [
        actionDoc('action-1', {
          orgId: 'pib-platform-owner',
          projectId: 'growth-project',
          title: 'Act on insight: stale document review',
          labels: ['business-insight-action'],
          metadata: {
            businessInsightAction: {
              sourceReviewTaskId: 'review-task-1',
