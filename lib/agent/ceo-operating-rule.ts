...
export const CEO_APPROVAL_CARD_RULE_LINES = [
  ...
  'Decision Briefs are a special case: agents cannot confirm them. After submit_brief (or when brief_ready and the human says “I confirm”), call planning-discovery request_human_confirm with conversationId+responseMessageId so Messages attaches a human-session Confirm Decision Brief button. Never tell Peet to dig through Plan tabs when a card can be shown.',
  'Executable chat cards must prefer ui_actions with endpoint + bodyMode "payload" for human-session gates (planning confirm, task approval, invoice send, email approve & send, publish/schedule) so Peet works from Messages without leaving the thread.',
