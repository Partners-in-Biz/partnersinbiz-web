...
export const FINANCE_HTTP_ENTRYPOINTS = [
  ...
  'app/api/v1/finance/revenue-recognition/commands/route.ts',
  'app/api/v1/finance/revenue-recognition/queries/route.ts',
...
export const SERVICE_ONLY_FINANCE_MODULES = [
  ...
  'lib/finance/expense-claims/service.ts',
  ...
  'lib/accounting/revenue-recognition-service.ts',
] as const
...
export const FINANCE_UI_BOUNDARY_NOTE =
  'Finance foundation workbench shipped under /portal/finance (overview, setup, ledger, reports, tax, documents, intercompany, payroll, ESS mobile/PWA employee self-service, personal, cross-org, statements, bank-rules, bank-feeds, budgets, cutover, packaging, multi-currency, practice, assets, inventory, job-costing). Operator depth (filters/saved views/bulk plan audit/multi-allocate/period-close blockers) and interactive portal workbenches load live bundles and run authenticated commands for foundation, reports, tax, documents (including AR/AP depth: credit/debit notes, recurring schedules, statement export drafts, bulk issue/void/allocate, aging, attachments, portal filters), intercompany, payroll, personal books, cross-org payment notify/confirm, statement import + human-gated recon suggestions, bank rules that only emit human-gated recon suggestions (never auto-post, never initiate payment), mock-first bank feed connector (adapter + sync + human accept/dismiss only; noEgress in unit tests; no paid open-banking vendor), budgets/forecasts/cashflow planner (planning only), opening trial balance / cutover activation, Phase-3 download packaging (SARS-ready packs, payment instruction exports, accountant packs), multi-currency FX rate sets / revaluation / functional reports, practice workspace (role matrix, multi-client switcher, firm→client grants prepare/review/file-export with audit+revoke, practice attention queue, operator notifications, audit explorer), fixed assets, lite revenue recognition (straight-line/milestone deferred revenue linked to AR/contracts, period recognition runs with reverse/audit, deferred vs billed reports), inventory stock lite + COGS on invoice issue (SKU master, qty movements from bills/invoices when tracked, stock on hand + audited adjustments; no WMS/POS/barcode), job costing, expense claims (draft to submit to approve/reject to post with VAT lines, receipts, optional OCR assist never auto-applied, bulk approve, observe-only payment instruction export), mobile/PWA ESS (employee-scoped payslip list/download + leave balance/request with approval routing into existing leave model; least privilege, no admin payroll controls, no mass email), and Phase 5 proving kit (demo seed / close fixture / packaging dry-run / accountant checklist) (project dimensions, project P&L/WIP, optional time costing without double-billing). SARS submission and external payment initiation remain disabled. Personal books are owner-private. Cross-org notices require CRM linkedOrgId or active businessRelationships. Statement suggestions never auto-post. Counterparty statements are draft/export only with massEmailAllowed=false. Cutover requires balanced TB + AR/AP open-item recon + approval before book.cutoverAt. Packaging is download/manifest only with externalEgressAllowed=false. Multi-currency uses immutable approved accounting_rate_sets (not operational fx_rates), realized FX on settlement, and balanced period-end revaluation journals with optional reverse-next-period. Practice switcher only lists orgs the user already belongs to and keeps X-Org-Id tenant scope. Firm→client practice grants add scoped prepare/review/file-export without membership sprawl; grant access is audited; revoke is immediate; clientVisibleMessagesAllowed and externalEgressAllowed stay false. Job costing applications propose journal/invoice lines only and refuse double-billing claims. Sensitive access is enforced at policy + service boundaries.'
