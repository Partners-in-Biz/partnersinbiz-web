...
function toTicket(id: string, data: RawDoc, orgName?: string): SupportTicket {
  return serializeSupportValue({
    ...
    requesterOrgId: data.requesterOrgId ?? data.orgId ?? null,
    providerOrgId: data.providerOrgId ?? null,
    partnerLinkId: data.partnerLinkId ?? null,
    scopeAgreementId: data.scopeAgreementId ?? null,
...
export async function createSupportTicket(args: {
  ...
  const ticket = {
    orgId: args.orgId,
    requesterOrgId: args.orgId,
    providerOrgId: null,
    partnerLinkId: null,
    scopeAgreementId: null,
    participants: [{
      id: `requester:${args.uid}`,
      userId: args.uid,
      orgId: args.orgId,
      role: 'requester',
      status: 'active',
  ...
  batch.set(messageRef, {
    ...
    kind: 'comment' satisfies SupportMessageKind,
    visibility: 'shared',
    attachments: [],
    contextRefs: args.contextRefs ?? [],
    createdAt: FieldValue.serverTimestamp(),
