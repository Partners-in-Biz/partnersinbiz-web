...
- For SEO/client website reports, prefer `/admin/reports` with the selected client and property when the readout is property-specific. The report snapshot uses first-party `product_sessions` and `product_events` as fallback web KPIs for sessions, pageviews, users, and conversions when no metrics fact rows exist.
